...
function promptForRound(doc, label, round) {
  ...
  const extras = {
    11: '请继续做最终缺口审计：逐项检查 manifest、workspace_snapshot、validation_report、预检 UI、导出历史 API、敏感信息剔除、测试覆盖和构建结果。只修明显缺口，不扩大范围，并给出具体文件级说明。',
    12: '请做一次端到端交付验收：用最小示例数据说明导出前预检、导出包生成、manifest hash、缺失资产记录、后端审计写入和历史查询的完整链路。发现未实现或失败项请直接补齐并重新运行验证。',
  };
  if (extras[round]) return extras[round];
  return extras[12];
}
...
async function appendRound(taskId, modelId, round, prompt) {
  const id = newTaskId();
  const payload = {
...
for (let i = 1; i <= iterations; i += 1) {
  ...
  for (const taskId of taskIds) {
    ...
    for (const summary of summaries) {
      ...
      if (busyStatuses.has(summary.status) || !terminalStatuses.has(summary.status)) continue;
      const nextRound = Math.min(summary.latestRound + 1, maxRound);
      ...
      try {
        const result = await appendRound(taskId, summary.modelId, nextRound, prompt);
