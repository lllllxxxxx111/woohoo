...
import { createBudgetApi } from './serverApi.budget';
...
async function parseResponse<T>(response: Response): Promise<T> {
  ...
  if (!response.ok) {
    ...
    // Detect budget exceeded errors and trigger a budget refresh
    if (
      parsed &&
      typeof parsed === 'object' &&
      'errorCode' in parsed &&
      parsed.errorCode === 'BUDGET_EXCEEDED' &&
      typeof window !== 'undefined'
    ) {
      window.dispatchEvent(new Event('woohoo:budget-refresh'));
    }
...
export const getBudgetStatus = budgetApi.getStatus;
export const updateBudgetSettings = budgetApi.updateSettings;
export const listBudgetBlocks = budgetApi.listBlocks;
