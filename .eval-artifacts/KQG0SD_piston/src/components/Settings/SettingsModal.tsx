...
export const SettingsModal: React.FC = () => {
  ...
  const { credits, loading: creditsLoading, error: creditsError, reload: reloadCredits } =
    useBillingCredits();
  const { status: budgetStatus } = useBudget();
            ...
            {activeTab === 'overview' && (
              <Space direction="vertical" size="large" style={{ width: '100%' }}>
                ...
                <div className={styles.quickActions}>
                  ...
                  <button type="button" className={styles.actionTile} onClick={() => setActiveTab('budget')}>
                    <Gauge size={18} />
                    <span>
                      <strong>预算控制{budgetStatus?.level === 'blocked' ? ' · 已超限' : budgetStatus?.level === 'warning' ? ' · 接近上限' : ''}</strong>
                      <small>
                        {budgetStatus?.settings.enabled
                          ? budgetStatus.daily.hasLimit || budgetStatus.monthly.hasLimit
                            ? `日 ${Math.round(budgetStatus.daily.usageRatio * 100)}% / 月 ${Math.round(budgetStatus.monthly.usageRatio * 100)}%`
                            : '已启用，未设置上限'
                          : '未启用'}
                      </small>
                    </span>
