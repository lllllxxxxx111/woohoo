...
pub async fn ai_chat(
    ...
    crate::billing::budget_enforce::enforce_budget(
        ...
        &user_id.0,
        estimated_cost,
        crate::billing::budget_enforce::TASK_TYPE_CHAT,
        Some(&context.model),
...
pub async fn ai_chat_stream(
    ...
    crate::billing::budget_enforce::enforce_budget(
        ...
        &user_id.0,
        estimated_cost,
        crate::billing::budget_enforce::TASK_TYPE_STREAM,
        Some(&context.model),
        Some(&context.conversation.project_id),
    )
    .await?;
...
pub async fn enqueue_ai_task_for_request(
    ...
    // 预算检查（基于用户消息长度估算，异步任务在入队前拦截）
    let estimated_input_chars = context.content.chars().count() as i64 + 200; // 加粗略的 system prompt 开销
    let estimated_cost = crate::billing::budget_enforce::estimate_chat_cost(
    ...
    crate::billing::budget_enforce::enforce_budget(
        ...
        user_id,
        estimated_cost,
        crate::billing::budget_enforce::TASK_TYPE_TASK,
        Some(&context.model),
        Some(&context.conversation.project_id),
    )
    .await?;
