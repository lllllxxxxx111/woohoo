...
import { isBudgetExceededError } from '../../lib/serverApi';
import {
  createId,
  ...
  const sendAiMessage = useCallback(
    async (content: string, options?: SendMessageOptions): Promise<SendMessageResult> => {
      ...
      try {
        ...
        const isBudgetExceeded = isBudgetExceededError(error);
        ...
        // 预算超限错误的特殊处理
        const errorContent = isBudgetExceeded
          ? `⚠️ ${message}\n\n请前往「设置 > 预算控制」调整预算上限，或等待日/月预算周期重置。`
          : `AI 调用失败：${message}`;
        ...
        updateMessageLocally(
          ...
          (currentMessage) => ({
            ...
            meta: {
              ...
              ...(isBudgetExceeded ? { errorType: 'budget_exceeded' } : {}),
            },
          }),
        );
