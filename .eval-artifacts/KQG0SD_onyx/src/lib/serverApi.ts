...
async function parseResponse<T>(response: Response): Promise<T> {
  ...
  if (!response.ok) {
    ...
    // 检查是否是预算超限错误
    const errorCode = parsed && typeof parsed === 'object' && 'errorCode' in parsed 
      ? (parsed as Record<string, unknown>).errorCode as string 
      : undefined;
    
    const error = new Error(appendRequestId(errorMessage, requestId)) as Error & { errorCode?: string };
    if (errorCode) {
...
// 预算 API
import type { BudgetBlock, BudgetStatus, UpdateBudgetConfigInput } from '../types';
 ...
 * 更新当前用户的预算配置
 */
export async function updateBudgetConfig(input: UpdateBudgetConfigInput): Promise<BudgetStatus> {
  return requestApi<BudgetStatus>('/api/billing/budget', {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
 ...
 * 获取最近的预算拦截记录
 */
export async function listBudgetBlocks(): Promise<BudgetBlock[]> {
  return requestApi<BudgetBlock[]>('/api/billing/budget/blocks', {
    method: 'GET',
  });
}
 ...
 * 检查是否是预算超限错误
 */
export function isBudgetExceededError(error: unknown): error is Error & { errorCode?: string } {
  if (!(error instanceof Error)) return false;
  
  // 首先检查 errorCode
  const errorWithCode = error as Error & { errorCode?: string };
  if (errorWithCode.errorCode === 'BUDGET_EXCEEDED') {
    return true;
  }
  
  // 降级检查错误消息中是否包含预算超限相关关键词
  const message = error.message.toLowerCase();
  return (
    message.includes('预算超限') ||
    message.includes('budget exceeded') ||
    message.includes('daily_exceeded') ||
    message.includes('monthly_exceeded')
  );
}
