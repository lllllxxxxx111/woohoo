...
async fn run_schema_migrations(pool: &SqlitePool) -> Result<Vec<String>, sqlx::Error> {
    ...
    for (version, migration_sql) in [
        ...
        (
            "020_pipeline_manual_reviews",
            include_str!("../migrations/020_pipeline_manual_reviews.sql"),
        ),
    ] {
    ...
    if let Some(version) = run_pipeline_events_check_backfill_migration(pool).await? {
        applied_versions.push(version);
    }
...
/**
 * 022_pipeline_events_check_backfills
 *
 * 扩展 pipeline_run_events.event_type 的 CHECK 约束，把 manual_review 加入枚举。
 * SQLite 不支持 ALTER TABLE ... MODIFY CONSTRAINT，所以采用“重命名 -> 新建 -> 迁移 -> 清理”的策略。
 *
 * 仅当旧表存在且其 CHECK 不包含 manual_review 时才触发；新库（002 已经是新约束）是 no-op。
 */
async fn run_pipeline_events_check_backfill_migration(
    pool: &SqlitePool,
) -> Result<Option<String>, sqlx::Error> {
    const VERSION: &str = "022_pipeline_events_check_backfills";
    ...
    let table_exists: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'pipeline_run_events'",
    )
    .fetch_one(pool)
    .await?;
    if table_exists == 0 {
        // 全新库由 002 直接创建带 manual_review 的表，无需补数
        record_schema_migration(pool, VERSION, "rust").await?;
        return Ok(Some(VERSION.to_string()));
    }

    // 检测旧 CHECK 是否已经包含 manual_review（通过 sqlite_master.sql）
    let create_sql: Option<String> =
        sqlx::query_scalar("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'pipeline_run_events'")
            .fetch_one(pool)
            .await?;
    if let Some(sql) = create_sql {
        if sql.contains("manual_review") {
            record_schema_migration(pool, VERSION, "rust").await?;
            return Ok(Some(VERSION.to_string()));
        }
    }

    tracing::info!(version = VERSION, "扩展 pipeline_run_events.event_type CHECK 约束");

    // 1. 重命名旧表
    sqlx::raw_sql(
        "ALTER TABLE pipeline_run_events RENAME TO pipeline_run_events_legacy_v22;",
    )
    .execute(pool)
    .await?;
    ...
    sqlx::raw_sql(
        "INSERT INTO pipeline_run_events (id, run_id, step_id, event_type, payload_json, source, created_at)
         ...
         FROM pipeline_run_events_legacy_v22",
    )
    .execute(pool)
    .await?;

    // 4. 删除旧表
    sqlx::raw_sql("DROP TABLE pipeline_run_events_legacy_v22")
        .execute(pool)
        .await?;
