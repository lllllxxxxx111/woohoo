 ...
 * 聚合当前用户下 failed/blocked 步骤、需要人工复核或有 prompt optimization 的步骤
 */
pub async fn list_review_queue(
    ...
    // Build the WHERE clause for steps that need review:
    // - step status is 'failed' or 'blocked'
    // - OR run has error_code = 'MANUAL_REVIEW_REQUIRED'
    let mut conditions = vec![
        "r.user_id = ?".to_string(),
        "(s.status IN ('failed', 'blocked') OR r.error_code = 'MANUAL_REVIEW_REQUIRED')".to_string(),
    ];
    let mut values: Vec<String> = vec![user_id.0.clone()];
    ...
    // Count total matching (run_id, step_id) pairs
    let count_query = format!(
        "SELECT COUNT(*) FROM pipeline_runs r
         JOIN pipeline_run_steps s ON s.run_id = r.id
         WHERE {}",
        where_clause
    );
    ...
    // Fetch matching steps with run_id, ordered by updated_at
    let steps_query = format!(
        "SELECT s.id as step_id, s.run_id
    ...
    let mut steps_q = sqlx::query_as::<_, (String, String)>(&steps_query);
    for v in &values {
        steps_q = steps_q.bind(v);
    }
    steps_q = steps_q.bind(limit).bind(offset);
    let step_pairs: Vec<(String, String)> = steps_q.fetch_all(&state.db).await?;

    // Batch fetch runs and steps
    ...
    for (step_id, run_id) in &step_pairs {
        ...
        // Fetch project name and conversation title
        let project_name: Option<String> = sqlx::query_as::<_, (String,)>(
        ...
        let conversation_title: Option<String> = sqlx::query_as::<_, (String,)>(
        ...
        // Fetch latest event for this step
        let latest_event = sqlx::query_as::<_, PipelineRunEvent>(
            "SELECT * FROM pipeline_run_events WHERE run_id = ? AND step_id = ? ORDER BY created_at DESC LIMIT 1"
        )
        .bind(run_id)
        .bind(step_id)
        .fetch_optional(&state.db)
        ...
        // Fetch latest error event for this step
        let latest_error_event = sqlx::query_as::<_, PipelineRunEvent>(
            "SELECT * FROM pipeline_run_events WHERE run_id = ? AND step_id = ? AND event_type IN ('step_failed', 'failed') ORDER BY created_at DESC LIMIT 1"
        )
        .bind(run_id)
        .bind(step_id)
        .fetch_optional(&state.db)
        ...
        // Count prompt optimizations for this step
        let (opt_count,): (i64,) = sqlx::query_as(
            "SELECT COUNT(*) FROM pipeline_prompt_optimizations WHERE run_id = ? AND step_id = ?"
        )
        .bind(run_id)
        .bind(step_id)
        .fetch_one(&state.db)
        ...
        // Count manual reviews for this step
        let (rev_count,): (i64,) = sqlx::query_as(
            "SELECT COUNT(*) FROM pipeline_manual_reviews WHERE run_id = ? AND step_id = ?"
        )
        .bind(run_id)
        .bind(step_id)
        .fetch_one(&state.db)
        ...
        // Latest review for this step
        let latest_review = sqlx::query_as::<_, PipelineManualReview>(
            "SELECT * FROM pipeline_manual_reviews WHERE run_id = ? AND step_id = ? ORDER BY created_at DESC LIMIT 1"
        )
        .bind(run_id)
        .bind(step_id)
        .fetch_optional(&state.db)
 ...
 *
 * decision 支持:
 * - retry: 重试失败/阻塞步骤（复用 retry-step 语义），写入复核记录和事件
 * - cancel: 取消整个流程（将 run 置为 cancelled 终态），写入复核记录和事件
 * - acknowledge: 仅记录已知晓，不改变运行状态
 *
 * 注意：不支持 skip。跳过失败步骤会破坏依赖链（下游步骤依赖上游完成），
 * 如果不需要继续，应该使用 cancel 终止流程。
 */
pub async fn submit_review_decision(
    ...
    // Validate decision
    if !["retry", "cancel", "acknowledge"].contains(&decision.as_str()) {
        return Err(AppError::Validation(
            format!(
                "不支持的复核决定: {}。支持的决定: retry, cancel, acknowledge",
                decision
            )
            .into(),
        ));
    ...
    // Verify run ownership and get run
    ...
    // Reject actions on completed/cancelled runs (except acknowledge which is always valid)
    if ["completed", "cancelled"].contains(&run.status.as_str()) && decision != "acknowledge" {
        return Err(AppError::Validation(
            format!(
                "流程已处于终态 ({})，无法执行 {} 操作。可使用 acknowledge 记录已知晓。",
                run.status, decision
            )
            .into(),
        ));
    ...
    // Record a pipeline event for audit
    log_pipeline_event(
        &state.db,
        &run_id,
        Some(&step_id),
    ...
    match decision.as_str() {
        "retry" => {
            // Retry semantics: step must be failed/blocked, run must be running/paused/failed
            if !["failed", "blocked"].contains(&step.status.as_str()) {
                return Err(AppError::Validation(
                    format!("步骤当前状态 ({}) 不允许重试，仅 failed/blocked 步骤可重试", step.status).into(),
                ));
            }
            if !["running", "paused", "failed"].contains(&run.status.as_str()) {
                return Err(AppError::Validation(
                    format!("流程当前状态 ({}) 不允许重试步骤", run.status).into(),
                ));
            }

            // Perform the retry (same logic as retry_pipeline_step)
            updated_step = Some(
                sqlx::query_as::<_, PipelineRunStep>(
                    "UPDATE pipeline_run_steps
                     SET status = 'retrying',
                         ai_task_id = NULL,
                         completed_at = NULL,
            ...
            // If run was failed, bring it back to running
            if run.status == "failed" {
                let run_upd = sqlx::query_as::<_, PipelineRun>(
                    "UPDATE pipeline_runs
                     SET status = 'running',
                         error_message = NULL,
                         error_code = NULL,
            ...
            log_pipeline_event(
                ...
                &serde_json::json!({
                    "stepId": step_id,
                    "stepName": step.step_name,
                    "reason": format!("manual_review: {}", note.as_deref().unwrap_or("")),
                    "reviewId": review_id,
                }),
                "user",
            )
        ...
        "cancel" => {
            // Cancel the entire run: transition to cancelled (terminal state)
            let (run_upd, changed) = cancel_pipeline_run_state(&state.db, &user_id.0, &run_id).await?;
            updated_run = Some(run_upd);
            ...
            if changed {
                log_pipeline_event(
                    ...
                    "cancelled",
                    &serde_json::json!({
                        "reason": format!("manual_review_cancel: {}", note.as_deref().unwrap_or("")),
                        "stepId": step_id,
                        "stepName": step.step_name,
                        "reviewId": review_id,
                    }),
        ...
        "acknowledge" => {
            // Just record the review; no state changes
            // The review record and event are already written above
        }
        _ => unreachable!(),
    }
...
/**
 * 获取指定步骤的复核记录列表
 * GET /api/pipelines/runs/{runId}/steps/{stepId}/reviews
 */
pub async fn list_step_reviews(
    ...
    // Verify step belongs to run
    let _step = sqlx::query_as::<_, PipelineRunStep>(
        "SELECT * FROM pipeline_run_steps WHERE id = ? AND run_id = ?",
    )
    .bind(&step_id)
    .bind(&run_id)
    ...
    let reviews = sqlx::query_as::<_, PipelineManualReview>(
        "SELECT * FROM pipeline_manual_reviews WHERE run_id = ? AND step_id = ? ORDER BY created_at DESC LIMIT 50",
    )
    .bind(&run_id)
    .bind(&step_id)
    .fetch_all(&state.db)
...
mod tests {
    ...
    #[test]
    fn review_queue_filter_parses_query_params() {
        let uri: Uri = "/api/pipelines/review-queue?projectId=proj-1&status=failed&pipelineType=outline&limit=10&offset=0"
            .parse()
            .expect("valid test uri");
        let Query(filter) =
            Query::<super::ReviewQueueFilter>::try_from_uri(&uri).expect("query should parse");
    ...
    #[test]
    fn review_queue_filter_accepts_snake_case_aliases() {
        let uri: Uri = "/api/pipelines/review-queue?project_id=proj-2&pipeline_type=one_click"
            .parse()
            .expect("valid test uri");
        let Query(filter) =
            Query::<super::ReviewQueueFilter>::try_from_uri(&uri).expect("query should parse");
        ...
        assert_eq!(filter.project_id.as_deref(), Some("proj-2"));
        assert_eq!(filter.pipeline_type.as_deref(), Some("one_click"));
    }

    // ─── Review Decision Validation Tests ───────────────────────────

    #[tokio::test]
    async fn review_decision_validates_decision_value() {
        use super::PipelineReviewDecisionReq;
        use axum::Json;

        // Invalid decision should be rejected by the handler logic.
        // We test at the request parsing + validation level:
        let req = PipelineReviewDecisionReq {
            decision: "skip".to_string(), // not supported
            note: Some("test".to_string()),
        };
        assert!(req.decision != "retry" && req.decision != "cancel" && req.decision != "acknowledge");

        let req2 = PipelineReviewDecisionReq {
            decision: "retry".to_string(),
            note: None,
        };
        assert_eq!(req2.decision, "retry");
    }

    async fn setup_test_run_with_step(
        run_status: &str,
        step_status: &str,
    ) -> (SqlitePool, String, String, String, std::path::PathBuf) {
        let (pool, user_id, run_id, db_path) = setup_test_run(run_status).await;

        let step_id = format!("step-{}", Uuid::new_v4());
        sqlx::query(
            "INSERT INTO pipeline_run_steps (
                id, run_id, step_key, step_name, step_order,
                step_type, depends_on_json, max_retries, run_version, status, attempt_count
            ) VALUES (?, ?, 'test_step', 'Test Step', 1, 'design', '[]', 3, 1, ?, 0)",
        )
        .bind(&step_id)
        .bind(&run_id)
        .bind(step_status)
        ...
        (pool, user_id, run_id, step_id, db_path)
    }

    #[tokio::test]
    async fn review_decision_reject_invalid_decision() {
        let (pool, user_id, run_id, step_id, db_path) =
            setup_test_run_with_step("running", "failed").await;

        // Call get_run_by_id to verify setup worked (ownership check passes)
        let run = super::get_run_by_id(&pool, &user_id, &run_id)
            .await
        ...
        let step: Option<(String,)> =
            sqlx::query_as("SELECT id FROM pipeline_run_steps WHERE id = ? AND run_id = ?")
                .bind(&step_id)
                .bind(&run_id)
                .fetch_optional(&pool)
                .await
                .expect("query should work");
        assert!(step.is_some(), "step should exist");
    ...
    #[tokio::test]
    async fn review_decision_rejects_cancel_on_completed_run() {
        let (pool, user_id, run_id, _step_id, db_path) =
            setup_test_run_with_step("completed", "completed").await;

        // Cancelling a completed run should fail validation
        let result = super::cancel_pipeline_run_state(&pool, &user_id, &run_id).await;
        assert!(result.is_err(), "cancel from completed should fail");
    ...
    #[tokio::test]
    async fn manual_review_insert_and_fetch() {
        let (pool, user_id, run_id, step_id, db_path) =
            setup_test_run_with_step("failed", "failed").await;

        // Insert a manual review
        let review_id = format!("rev-{}", Uuid::new_v4());
        let now = chrono::Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, true);
        sqlx::query(
            "INSERT INTO pipeline_manual_reviews (id, user_id, run_id, step_id, decision, note, created_at)
             VALUES (?, ?, ?, ?, 'acknowledge', 'test note', ?)"
        )
        .bind(&review_id)
        .bind(&user_id)
        .bind(&run_id)
        ...
        .execute(&pool)
        .await
        .expect("insert review should work");

        // Fetch it back
        let review: super::PipelineManualReview = sqlx::query_as(
            "SELECT * FROM pipeline_manual_reviews WHERE id = ?"
        )
        .bind(&review_id)
        .fetch_one(&pool)
        .await
        .expect("fetch review should work");

        assert_eq!(review.decision, "acknowledge");
        assert_eq!(review.note.as_deref(), Some("test note"));
        assert_eq!(review.run_id, run_id);
        assert_eq!(review.step_id, step_id);
