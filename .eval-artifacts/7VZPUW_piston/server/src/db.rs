...
async fn run_schema_migrations(pool: &SqlitePool) -> Result<Vec<String>, sqlx::Error> {
    ...
    if let Some(version) = run_pipeline_manual_reviews_backfill_migration(pool).await? {
        applied_versions.push(version);
    }
...
/** 确保 pipeline_manual_reviews 表存在（为早期数据库回填） */
async fn run_pipeline_manual_reviews_backfill_migration(
    pool: &SqlitePool,
) -> Result<Option<String>, sqlx::Error> {
    const VERSION: &str = "021_pipeline_manual_reviews_backfill";

    if has_schema_migration(pool, VERSION).await? {
        return Ok(None);
    }

    tracing::info!(version = VERSION, "执行 pipeline_manual_reviews 回填");
    ensure_pipeline_manual_reviews_schema(pool).await?;
    record_schema_migration(pool, VERSION, "rust").await?;
    Ok(Some(VERSION.to_string()))
}

async fn ensure_pipeline_manual_reviews_schema(pool: &SqlitePool) -> Result<(), sqlx::Error> {
    sqlx::raw_sql(
