...
export function generateReadmeExport(manifest: ExportManifest): string {
  ...
  lines.push('2. For each file listed under "File listing" (data/, assets/, workspace_snapshot.json,');
  lines.push('   README_EXPORT.md, validation_report.json), compute SHA-256 and compare to the recorded hash.');
  lines.push('3. `validation_report.json` provides a machine-readable summary: counts, preflight');
  lines.push('   issues, and the final manifest hash. It is also covered by the file-list above.');
  lines.push('4. To verify `manifest.json` itself: load it, remove the `manifestSha256` field,');
  lines.push('   re-serialize to JSON with sorted keys and no whitespace, compute SHA-256, and');
  lines.push('   confirm it matches the value that was in `manifestSha256`.');
  lines.push('   (manifest.json is intentionally NOT listed in the files table to avoid circular');
  lines.push('   self-reference.)');
  lines.push('');
