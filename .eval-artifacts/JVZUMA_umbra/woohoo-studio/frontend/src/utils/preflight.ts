 ...
 *  - blob:  (in-browser object URLs)
 *  - data:  (inlined data URLs)
 *  - local:// (pseudo-scheme for uploaded-not-yet-saved assets)
 */
function assetUrlLooksBroken(asset: Asset): boolean {
  ...
  if (lower.startsWith('blob:') || lower.startsWith('data:') || lower.startsWith('local:')) return false;
  // Any other shape is suspect.
  return true;
}

/**
 * Returns true if a recorded sizeBytes indicates an empty/unusable asset (0 or
 * negative). Undefined size is NOT treated as broken — many external URLs
 * don't have size recorded up-front.
 */
function assetIsEmpty(asset: Asset): boolean {
  return typeof asset.sizeBytes === 'number' && asset.sizeBytes <= 0;
}
...
export function runPreflightChecks(
  ...
  for (const asset of assets) {
    ...
    // Empty asset file (zero or negative recorded size) is unusable for export.
    if (assetIsEmpty(asset)) {
      issues.push({
        severity: 'blocking',
        code: 'ASSET_EMPTY',
        message: `Asset "${asset.name}" has size ${asset.sizeBytes} bytes; cannot pack an empty file.`,
        entityType: 'asset',
        entityId: asset.id,
        detail: String(asset.sizeBytes),
      });
    }
  }
  ...
  // Storyboard-level: a storyboard with no shots has no content.
  for (const sb of storyboards) {
    ...
    if (shots.length === 0) {
      issues.push({
        ...
        message: `Storyboard "${sb.name}" has no shots.`,
    ...
    for (const shot of shots) {
      if (!shot.description || shot.description.trim().length === 0) {
        issues.push({
          severity: 'warning',
          code: 'SHOT_EMPTY_DESCRIPTION',
          message: `Shot #${shot.number} in storyboard "${sb.name}" has no description.`,
          entityType: 'shot',
          entityId: shot.id,
        });
      }
  ...
  // Scripts with no scenes AND no free-form content are effectively empty.
  for (const script of scripts) {
    const hasScenes = Array.isArray(script.scenes) && script.scenes.length > 0;
    const hasContent = !!script.content && script.content.trim().length > 0;
    ...
    } else if (!hasScenes && hasContent) {
      issues.push({
        severity: 'info',
        code: 'SCRIPT_NO_SCENES',
        message: `Script "${script.title}" has content but no structured scenes.`,
        entityType: 'script',
        entityId: script.id,
      });
    }
  ...
  // Keyframes referencing non-existent asset IDs, and orphan keyframes (no assetId at all).
  const assetIds = new Set(assets.map((a) => a.id));
  for (const kf of keyframes) {
    if (!kf.assetId) {
      issues.push({
        severity: 'warning',
        code: 'KEYFRAME_NO_ASSET',
        message: `Keyframe "${kf.name}" has no associated asset.`,
        entityType: 'keyframe',
        entityId: kf.id,
      });
    } else if (!assetIds.has(kf.assetId)) {
      issues.push({
        severity: 'warning',
    ...
    // Info: keyframe with neither prompt nor annotations — likely a placeholder.
    const hasPrompt = !!kf.prompt && kf.prompt.trim().length > 0;
    const hasNotes = !!kf.annotations && kf.annotations.trim().length > 0;
    if (!hasPrompt && !hasNotes) {
      issues.push({
        severity: 'info',
        code: 'KEYFRAME_NO_NOTES',
        message: `Keyframe "${kf.name}" has no prompt or annotations.`,
        entityType: 'keyframe',
        entityId: kf.id,
      });
    }
  ...
  // Video plans with no model specified, or zero duration.
  for (const vp of videoPlans) {
    ...
    if (typeof vp.duration !== 'number' || vp.duration <= 0) {
      issues.push({
        severity: 'warning',
        code: 'VIDEO_PLAN_ZERO_DURATION',
        message: `Video plan "${vp.name}" has zero or invalid duration.`,
        entityType: 'videoPlan',
        entityId: vp.id,
      });
    }
    if (!vp.resolution || vp.resolution.width <= 0 || vp.resolution.height <= 0) {
      issues.push({
        severity: 'info',
        code: 'VIDEO_PLAN_BAD_RESOLUTION',
        message: `Video plan "${vp.name}" has no valid resolution set.`,
        entityType: 'videoPlan',
        entityId: vp.id,
      });
    }
