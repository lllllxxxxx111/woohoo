    ...
    async (content: string, options?: SendMessageOptions): Promise<SendMessageResult> => {
      ...
      try {
        if (isServerWorkspaceReady) {
          ...
          // If the POST response already carries a terminal status (extremely
          // fast failure / synchronous execution), finalize the placeholder
          // directly using the same content semantics as SSE, and let the SSE
          // stream de-duplicate via seen-event keys. This avoids a race where
          // we show a queued placeholder for a task that has already terminated.
          if (isTerminalStatus(task.status)) {
            const { content: terminalContent, role } = formatTerminalTaskContent(task);
            updateMessageLocally(
              targetProjectId,
              targetChatId,
              effectivePlaceholderMessageId,
