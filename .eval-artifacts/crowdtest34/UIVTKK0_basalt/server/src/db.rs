...
async fn run_schema_migrations(pool: &SqlitePool) -> Result<Vec<String>, sqlx::Error> {
    ...
    if let Some(version) = run_ai_task_result_backfill_migration(pool).await? {
        applied_versions.push(version);
    }
...
/** Backfill: add ai_tasks.result column for final task output persistence.
 *  Without this column, completed tasks lose their result on process restart
 *  and HTTP resync returns empty content. Idempotent: only adds column when missing. */
async fn run_ai_task_result_backfill_migration(
    pool: &SqlitePool,
) -> Result<Option<String>, sqlx::Error> {
    const VERSION: &str = "024_ai_task_result";

    if has_schema_migration(pool, VERSION).await? {
        return Ok(None);
    }

    tracing::info!(version = VERSION, "执行 ai_tasks.result 列回填");
    let columns = list_table_columns(pool, "ai_tasks").await?;
    if !columns.is_empty() && !columns.contains("result") {
