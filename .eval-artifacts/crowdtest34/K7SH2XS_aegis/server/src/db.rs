...
async fn run_schema_migrations(pool: &SqlitePool) -> Result<Vec<String>, sqlx::Error> {
    ...
    for (version, migration_sql) in [
        ...
        (
            "024_ai_routing_audit",
            include_str!("../migrations/024_ai_routing_audit.sql"),
        ),
    ] {
    ...
    if let Some(version) = run_routing_capability_backfill_migration(pool).await? {
        applied_versions.push(version);
    }
...
async fn run_routing_capability_backfill_migration(
    pool: &SqlitePool,
) -> Result<Option<String>, sqlx::Error> {
    const VERSION: &str = "025_routing_capability_backfill";

    if has_schema_migration(pool, VERSION).await? {
        return Ok(None);
    }

    tracing::info!(version = VERSION, "执行 AI 路由能力字段兼容回填");
    ensure_routing_capability_schema(pool).await?;
    record_schema_migration(pool, VERSION, "rust").await?;
    Ok(Some(VERSION.to_string()))
}

async fn ensure_routing_capability_schema(pool: &SqlitePool) -> Result<(), sqlx::Error> {
    let columns = list_table_columns(pool, "ai_endpoint_capabilities").await?;
    if !columns.is_empty() && !columns.contains("max_context_tokens") {
        sqlx::query(
            "ALTER TABLE ai_endpoint_capabilities ADD COLUMN max_context_tokens INTEGER",
        )
        .execute(pool)
        .await?;
    }
