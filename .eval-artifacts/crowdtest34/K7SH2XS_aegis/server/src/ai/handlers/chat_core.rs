...
use crate::ai::router::{RoutingOperation, RoutingRequest};
...
pub(super) async fn resolve_chat_context(
    ...
    // Use the smart router for endpoint selection with fallback support
    let routing_operation = match operation {
    ...
    // Determine if agent has an explicit endpoint that should be respected
    let effective_endpoint_id = req.endpoint_id.clone().or_else(|| {
        agent.as_ref().and_then(|a| a.endpoint_id.clone())
    });

    // If agent has an explicit model, use that as fallback
    let agent_model = agent.as_ref().and_then(|a| a.model.clone());

    let requires_stream = matches!(operation, AiUsageOperation::Stream | AiUsageOperation::Task);

    // Rough token estimate: ~4 chars per token, plus overhead for system prompt and resource refs
    let estimated_tokens = (content.chars().count() as i64) / 4 + 1024;

    let routing_request = RoutingRequest {
        user_id: user_id.to_string(),
        operation: routing_operation,
        explicit_endpoint_id: effective_endpoint_id,
        requested_model: req.model.clone().or(agent_model),
        context_length: Some(estimated_tokens),
        requires_stream,
    ...
    let primary = plan.primary().ok_or_else(|| {
        AppError::Validation("没有可用的 AI 端点，请先在设置中配置端点和能力".into())
    })?;

    let endpoint = primary.endpoint.clone();
    ...
    Ok(ResolvedChatContext {
        ...
        routing_candidate_index: 0,
        routing_explicit_endpoint_id: effective_endpoint_id,
        routing_requested_model: req.model.clone().or(agent_model),
        routing_requires_tools: req.allow_assistant_actions,
        routing_context_length: Some(estimated_tokens),
    })
}
