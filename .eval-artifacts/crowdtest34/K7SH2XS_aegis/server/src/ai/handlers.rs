...
use super::{
    ...
    router::{self, RoutingOperation, RoutingRequest},
...
struct ResolvedChatContext {
    ...
    // Routing fields for fallback and audit
    routing_request_id: String,
    routing_used_fallback: bool,
    routing_candidate_index: usize,
    /// Original explicit endpoint ID from request/agent (for fallback constraint)
    routing_explicit_endpoint_id: Option<String>,
    /// Original requested model from request/agent (for fallback constraint)
    routing_requested_model: Option<String>,
    /// Whether the request requires tool use support
    routing_requires_tools: bool,
    /// Estimated context length in tokens for routing decisions
    routing_context_length: Option<i64>,
}
...
/// Result of executing an AI chat call with possible fallback.
struct ChatCallResult {
    content: String,
    model: String,
    usage: Option<crate::ai::client::TokenUsage>,
    final_endpoint: AiEndpoint,
    final_model: String,
    used_fallback: bool,
    candidate_index: usize,
    attempts: Vec<router::RoutingAttempt>,
}

/// Execute a sync chat call with automatic fallback across routing candidates.
async fn execute_chat_with_fallback(
    state: &AppState,
    context: &ResolvedChatContext,
    messages: Vec<ChatMessage>,
) -> AppResult<ChatCallResult> {
    let routing_operation = match context.operation {
        AiUsageOperation::Chat => RoutingOperation::Chat,
        AiUsageOperation::Stream => RoutingOperation::Stream,
        AiUsageOperation::Task => RoutingOperation::Task,
        AiUsageOperation::Test => RoutingOperation::Test,
    };

    let routing_request = RoutingRequest {
        user_id: context.conversation.user_id.clone(),
        operation: routing_operation,
        explicit_endpoint_id: context.routing_explicit_endpoint_id.clone(),
        requested_model: context.routing_requested_model.clone(),
        context_length: context.routing_context_length,
        requires_stream: matches!(context.operation, AiUsageOperation::Stream | AiUsageOperation::Task),
        requires_tools: context.routing_requires_tools,
        ...
        allow_fallback: true,
        max_attempts: 3,
        request_id: Some(context.routing_request_id.clone()),
        ..Default::default()
    };

    let plan = router::build_routing_plan(&state.db, routing_request).await?;

    if plan.candidates.is_empty() {
        let event = router::RoutingEventRecord::new(
            &plan, None, 0, None, None,
            router::RoutingStatus::NoCandidate, None, None, None,
        );
        router::record_routing_event_safe(&state.db, &event).await;
        return Err(AppError::Validation("没有可用的 AI 端点".into()));
    }

    let mut attempts: Vec<router::RoutingAttempt> = Vec::new();
        ...
        match result {
            Ok(chat_result) => {
                ...
                let event = router::RoutingEventRecord::new(
                    ...
                    Some(&chat_result.model),
                    if idx > 0 { router::RoutingStatus::Fallback } else { router::RoutingStatus::Success },
                    None,
                    None,
                    Some(latency_ms),
                );
                ...
                attempts.push(router::RoutingAttempt {
                    candidate_index: idx,
                    endpoint_id: candidate.endpoint.id.clone(),
                    endpoint_name: candidate.endpoint.name.clone(),
                    model: candidate.model.clone(),
                    status: if idx > 0 { router::RoutingStatus::Fallback } else { router::RoutingStatus::Success },
                ...
                return Ok(ChatCallResult {
                    ...
                    final_endpoint: candidate.endpoint.clone(),
                    final_model: candidate.model.clone(),
                    used_fallback: idx > 0,
                    candidate_index: idx,
                    attempts,
                });
            ...
            Err(error) => {
                ...
                let classification = router::classify_error(&error_msg);
                ...
                attempts.push(router::RoutingAttempt {
                    ...
                    endpoint_name: candidate.endpoint.name.clone(),
                    model: candidate.model.clone(),
                    status: router::RoutingStatus::Failed,
                ...
                if !retryable || !plan.request.allow_fallback {
                    // Non-retryable error or fallback disabled
                    tracing::warn!(
                        endpoint = %candidate.endpoint.name,
                        ?classification,
                        retryable,
                        "Non-retryable error on candidate, stopping fallback chain"
                    );
                    break;
                }
                ...
                tracing::warn!(
                    ...
                    "Attempt failed, falling back to next endpoint"
                );
            }
        }
    }

    // All candidates exhausted — build a clear error message
    let error_msg = if attempts.len() > 1 {
        let failed_endpoints: Vec<String> = attempts.iter()
            .filter(|a| a.status == router::RoutingStatus::Failed)
            .map(|a| {
                let err = a.error_message.as_deref().unwrap_or("unknown error");
                // Truncate long error messages
                let short_err: String = err.chars().take(120).collect();
                format!("{}: {}", a.endpoint_name, short_err)
            })
            .collect();
        format!(
            "已尝试 {} 个 AI 端点均失败：{}",
        ...
        match last_error {
            ...
            None => "所有 AI 端点均不可用".to_string(),
        }
    };
    Err(last_error.unwrap_or_else(|| AppError::Internal(error_msg)))
}
...
pub async fn ai_chat(
    ...
    let chat_result = execute_chat_with_fallback(&state, &context, prepared.messages.clone()).await;
    ...
    // Update context with the actual endpoint/model used (may differ due to fallback)
    let mut context = context;
    let used_fallback = chat_result.used_fallback;
    if used_fallback {
        context.endpoint = chat_result.final_endpoint.clone();
        context.model = chat_result.final_model.clone();
        context.routing_used_fallback = true;
        context.routing_candidate_index = chat_result.candidate_index;
    }

    let usage =
        usage::usage_from_response(&prepared.messages, &chat_result.content, chat_result.usage.as_ref());
    let finalized =
        match finalize_assistant_response(&state, &user_id.0, &context, &chat_result.content).await {
            ...
            Err(error) => {
                ...
                record_usage_safe(
                    ...
                    build_usage_record(
                        ...
                        0,
                        prepared.input_chars,
                        chat_result.content.chars().count() as i64,
                        usage,
                        Some(error.to_string()),
                    ),
                )
    ...
    // Include fallback info in response for UI display
    if used_fallback {
        if let Some(obj) = response_obj.as_object_mut() {
            obj.insert("routingFallback".into(), serde_json::json!({
                "used": true,
                "attemptCount": chat_result.attempts.len(),
                "attempts": chat_result.attempts.iter().map(|a| serde_json::json!({
...
pub async fn ai_chat_stream(
    ...
    // Build routing plan for stream with fallback on initial connection
    let stream_routing_request = RoutingRequest {
        user_id: context.conversation.user_id.clone(),
        operation: RoutingOperation::Stream,
        explicit_endpoint_id: context.routing_explicit_endpoint_id.clone(),
        requested_model: context.routing_requested_model.clone(),
        context_length: context.routing_context_length,
    ...
    let stream_plan = router::build_routing_plan(&state.db, stream_routing_request).await;
    let (ai_stream, stream_endpoint_id, stream_model, stream_used_fallback, stream_attempt_count) = match stream_plan {
        Ok(plan) => {
            if plan.candidates.is_empty() {
                let event = router::RoutingEventRecord::new(
                    &plan, None, 0, None, None,
                ...
                router::record_routing_event_safe(&state.db, &event).await;
                let error = AppError::Validation("没有可用的 AI 端点".into());
                if let Err(message_error) =
                    save_failure_message(&state.db, &context, &error.to_string(), None).await
                {
                    tracing::warn!("Failed to persist stream no-candidate error: {}", message_error);
                }
                return Err(error);
            }

            let mut stream_result: Result<(_, String, String, usize), (AppError, usize)> = Err((AppError::Internal("no candidates".into()), 0));
            for (idx, candidate) in plan.candidates.iter().enumerate() {
                    ...
                    .await
                {
                    Ok(stream) => {
                        ...
                        router::record_routing_event_safe(&state.db, &event).await;
                        stream_result = Ok((stream, candidate.endpoint.id.clone(), candidate.model.clone(), idx));
                        break;
                    }
                    Err(error) => {
                        let error_msg = error.to_string();
                        ...
                        if classification.is_retryable() && plan.request.allow_fallback && idx + 1 < plan.candidates.len() {
                            tracing::warn!(
                                endpoint = %candidate.endpoint.name,
                                ?classification,
                                "Stream attempt failed, trying next endpoint"
                            );
                            continue;
                        }
                        stream_result = Err((error, idx));
                        break;
                    }
                }
            }
            ...
            match stream_result {
                Ok((stream, ep_id, model, success_idx)) => {
                    let used_fallback = success_idx > 0;
                    (stream, ep_id, model, used_fallback, success_idx + 1)
                }
                Err((error, _)) => {
                    if let Err(message_error) =
                        save_failure_message(&state.db, &context, &error.to_string(), None).await
        ...
        Err(plan_error) => {
            // If routing plan fails, fall back to original behavior with context endpoint
            let stream_result = state
                ...
                .await;
            match stream_result {
                Ok(stream) => (stream, context.endpoint.id.clone(), context.model.clone(), false, 1),
                Err(error) => {
                    if let Err(message_error) =
                        save_failure_message(&state.db, &context, &error.to_string(), None).await
                    {
    ...
    let endpoint_id = stream_endpoint_id;
    let provider = context.endpoint.provider.clone();
    let api_key = context.endpoint.api_key.clone();
    let model_for_save = stream_model;
    let stream_used_fallback = stream_used_fallback;
    let stream_attempt_count = stream_attempt_count;
    ...
    let sse_stream = async_stream::stream! {
        ...
        if let Some(error_message) = postprocess_error.as_ref() {
            ...
            if stream_used_fallback {
                let fallback_meta = serde_json::json!({
                    "routingFallback": {
                        "used": true,
                        "attemptCount": stream_attempt_count,
                    }
                });
                yield Ok(Event::default().event("meta").data(fallback_meta.to_string()));
            }
...
async fn run_ai_task_non_stream(
    ...
    // Build routing plan for fallback across endpoints
    let routing_request = RoutingRequest {
        user_id: context.conversation.user_id.clone(),
        operation: RoutingOperation::Task,
        explicit_endpoint_id: context.routing_explicit_endpoint_id.clone(),
        requested_model: context.routing_requested_model.clone(),
        context_length: context.routing_context_length,
    ...
    let plan = match router::build_routing_plan(&state.db, routing_request).await {
        Ok(plan) => plan,
        Err(error) => {
            fail_task_execution(
                state,
    ...
    let mut task_attempts: Vec<(String, String, Option<String>)> = Vec::new(); // (endpoint_name, model, error)
    ...
    for (idx, candidate) in plan.candidates.iter().enumerate() {
        ...
        match result {
            Ok(chat_result) => {
                ...
                task_attempts.push((candidate.endpoint.name.clone(), candidate.model.clone(), None));

                // Update context with actual endpoint/model used
                if idx > 0 {
                    actual_context.endpoint = candidate.endpoint.clone();
                    actual_context.model = candidate.model.clone();
                    actual_context.routing_used_fallback = true;
                    actual_context.routing_candidate_index = idx;
                }
            ...
            Err(error) => {
                ...
                task_attempts.push((candidate.endpoint.name.clone(), candidate.model.clone(), Some(error_msg.clone())));
                ...
                tracing::warn!(
                    endpoint = %candidate.endpoint.name,
                    ?classification,
                    "Task attempt failed, falling back to next endpoint"
                );
            }
        }
    }

    let error_msg = if task_attempts.len() > 1 {
        let failed: Vec<String> = task_attempts
            .iter()
            ...
            .collect();
        format!("已尝试 {} 个 AI 端点均失败：{}", task_attempts.len(), failed.join("; "))
    } else if let Some(ref err) = last_error {
        err.clone()
    } else {
        "所有 AI 端点均不可用".to_string()
    };
    fail_task_execution(
        state,
        user_id,
