...
export type RoutingEvent = {
  ...
  status: 'success' | 'failed' | 'fallback' | 'no_candidate';
  errorClassification?: string | null;
  errorMessage?: string | null;
  fallbackFromIndex?: number | null;
  attemptCount: number;
  maxAttempts: number;
...
export async function listRoutingEvents(params?: {
  endpointId?: string;
  capability?: string;
  status?: string;
  operation?: string;
  ...
  return requestApi<RoutingEvent[]>(`/api/ai/routing/events${qs ? `?${qs}` : ''}`);
}

export async function getRoutingHealth(hours = 24): Promise<RoutingHealthSummary> {
  return requestApi<RoutingHealthSummary>(`/api/ai/routing/health?hours=${hours}`);
}
