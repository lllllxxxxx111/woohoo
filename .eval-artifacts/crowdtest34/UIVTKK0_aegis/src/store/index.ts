    ...
    set((state) => ({
      activeState: (() => {
        const nextState: ActiveState = {
          ...
          projectId,
          currentTab:
            state.activeState.currentTab === 'preview' ? 'chat' : state.activeState.currentTab,
  ...
  updateAiSettings: (settings) =>
    set((state) => {
      ...
      const keepSelectedEndpoint =
        ...
        state.aiSettings.provider.trim().toLowerCase() ===
          nextSettings.provider.trim().toLowerCase() &&
