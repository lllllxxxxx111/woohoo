 ...
 *
 * Supports Last-Event-ID header for cursor-based replay.
 * Events carry an id field (the SQLite rowid) so clients can resume after disconnect.
 * When the run reaches a terminal state, a final event is emitted and the stream closes.
 */
pub async fn stream_pipeline_run(
    ...
    // Query param is provided for clients that can't set custom headers (native EventSource).
    ...
    let stream = async_stream::stream! {
        // If no cursor, send connected snapshot
        if initial_cursor == 0 {
        ...
        let mut last_real_rowid = initial_cursor; // tracks DB rowid; not advanced by synthetic error events
        let mut synthetic_id = initial_cursor; // monotonically increasing for synthetic event IDs
        let mut empty_polls = 0u32;
        let mut consecutive_db_errors = 0u32;

        // Helper to emit a batch of events and advance last_event_id
        macro_rules! emit_events {
            ($events:expr) => {
                for (event_type, payload, source, rowid) in $events {
                    last_event_id = last_event_id.max(rowid);
                    last_real_rowid = last_real_rowid.max(rowid);
                    synthetic_id = synthetic_id.max(rowid);
        ...
        // Helper to emit synthetic events (errors, done, etc.) with unique IDs
        macro_rules! emit_synthetic {
            ...
            ($event_type:expr, $data:expr, retry = $retry_secs:expr) => {{
                synthetic_id += 1;
                last_event_id = synthetic_id;
                yield Ok(Event::default()
                    .id(synthetic_id.to_string())
                    .event($event_type)
                    .retry(std::time::Duration::from_secs($retry_secs))
                    .data($data));
            }};
        }

        // On resume (cursor > 0), immediately fetch and replay missed events
        if initial_cursor > 0 {
            if let Ok(initial_events) = sqlx::query_as::<_, (String, String, String, i64)>(
                "SELECT pre.event_type, pre.payload_json, pre.source, pre.rowid
                 FROM pipeline_run_events pre
                 JOIN pipeline_runs pr ON pre.run_id = pr.id
                 WHERE pre.run_id = ? AND pr.user_id = ? AND pre.rowid > ?
                 ORDER BY pre.rowid ASC LIMIT 200"
            )
            .bind(&run_id)
            .bind(&user_id_clone)
            .bind(last_real_rowid)
            .fetch_all(&db)
            .await {
                emit_events!(initial_events);
            }
        }

        // Poll for up to ~20 minutes (600 * 2s). After that send a done signal
        // and close — client can reconnect with cursor to continue.
        for poll_count in 0..600 {
            tokio::time::sleep(std::time::Duration::from_secs(2)).await;

            // Fetch new events scoped to this run
            match sqlx::query_as::<_, (String, String, String, i64)>(
                "SELECT pre.event_type, pre.payload_json, pre.source, pre.rowid
                 FROM pipeline_run_events pre
                 JOIN pipeline_runs pr ON pre.run_id = pr.id
                 WHERE pre.run_id = ? AND pr.user_id = ? AND pre.rowid > ?
                 ORDER BY pre.rowid ASC LIMIT 50"
            )
            .bind(&run_id)
            .bind(&user_id_clone)
            .bind(last_real_rowid)
            ...
            .await {
                ...
                Err(err) => {
                    ...
                    emit_synthetic!("error", err_data);
                    if consecutive_db_errors >= 5 {
                        emit_synthetic!("done", "{\"reason\":\"db_error\"}".to_string());
                        return;
                    }
                    continue;
                }
            ...
            // Check if the run has reached a terminal state every ~10s
            if poll_count % 5 == 0 {
                // Drain any final events before terminal check
                if let Ok(drain) = sqlx::query_as::<_, (String, String, String, i64)>(
                    "SELECT pre.event_type, pre.payload_json, pre.source, pre.rowid
                     FROM pipeline_run_events pre
                     JOIN pipeline_runs pr ON pre.run_id = pr.id
                     WHERE pre.run_id = ? AND pr.user_id = ? AND pre.rowid > ?
                ...
                let terminal: Option<(String, Option<String>)> = sqlx::query_as(
                    "SELECT status, error_code FROM pipeline_runs WHERE id = ? AND user_id = ?"
                )
                .bind(&run_id)
                .bind(&user_id_clone)
                .fetch_optional(&db)
                ...
                if let Some((status, error_code)) = terminal {
                    if matches!(status.as_str(), "completed" | "failed" | "cancelled") {
                        let terminal_data = serde_json::json!({
                            "status": status,
                            "errorCode": error_code,
                        }).to_string();
                        emit_synthetic!("run_terminal", terminal_data);
                        let reason = if status == "completed" { "completed" } else { status.as_str() };
                        emit_synthetic!("done", serde_json::json!({"reason": reason}).to_string());
                        return;
                    }
                }
            }

            // After 10 minutes of inactivity (300 empty polls), send timeout done
            if empty_polls >= 300 {
                emit_synthetic!("done", "{\"reason\":\"timeout\"}".to_string(), retry = 30);
                return;
            }
        }

        emit_synthetic!("done", "{\"reason\":\"max_polls\"}".to_string());
    };
