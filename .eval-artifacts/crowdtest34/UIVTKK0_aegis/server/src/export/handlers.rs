...
pub async fn precheck(
            ...
            match fs::metadata(&file_path).await {
                ...
                Err(_) => {
                    ...
                    blocking_issues.push(PrecheckIssue {
                        ...
                        severity: "error",
                        message: format!("资产「{}」的本地文件缺失，导出时将被跳过", asset.name),
                        asset_id: Some(asset.id.clone()),
                    });
                }
            }
    ...
    if asset_summary.missing_or_broken == 0 && invalid_url_count == 0 && !assets.is_empty() {
        info.push(PrecheckIssue {
            ...
            severity: "info",
            message: format!("所有 {} 个资产链接有效，可正常打包", assets.len()),
            asset_id: None,
        });
    }

    if has_script && script_word_count >= MIN_SCRIPT_LENGTH_CHARS && storyboard_line_count > 0 {
        info.push(PrecheckIssue {
