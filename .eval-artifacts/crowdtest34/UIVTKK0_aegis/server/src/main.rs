...
async fn main() {
    ...
    // Clean up old persisted SSE events on startup to prevent unbounded DB growth
    if let Err(e) = ai::runtime::AiTaskRuntime::cleanup_old_persisted_events(&pool).await {
        tracing::warn!("清理旧 AI 任务事件失败: {}", e);
    }
