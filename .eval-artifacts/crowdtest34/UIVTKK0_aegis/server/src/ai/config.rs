...
pub struct AiTask {
    ...
    /// Monotonic event sequence number for cursor-based replay and out-of-order
    /// detection. Clients compare this against the SSE event id to detect stale
    /// API responses arriving after fresher SSE events.
    #[serde(default)]
    pub event_seq: i64,
}
...
pub struct AiTaskFilter {
    ...
    /// Resume from this event sequence number (cursor-based replay).
    /// Events with seq > cursor will be replayed from the buffer.
    #[serde(default)]
