 ...
 * 将任务状态持久化到数据库
 *
 * Uses UPSERT (ON CONFLICT DO UPDATE) to preserve event_seq from concurrent
 * updates from the publish path. INSERT OR REPLACE would delete+reinsert the
 * row, clobbering event_seq back to its default.
 */
pub async fn persist_task(
    ...
    sqlx::query(
        "INSERT INTO ai_tasks (
            ...
            active_tasks, queued_tasks, project_id, conversation_id, event_seq
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
...
pub async fn restore_tasks(db: &SqlitePool) -> Result<Vec<(AiTask, String)>, sqlx::Error> {
    ...
    for row in rows {
         ...
         */
        let original_status: AiTaskStatus =
            serde_json::from_str(&row.status).unwrap_or(AiTaskStatus::Failed);
