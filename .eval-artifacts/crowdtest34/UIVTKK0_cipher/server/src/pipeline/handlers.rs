 ...
 *
 * Supports Last-Event-ID header for cursor-based resume. Each event carries
 * an `id:` field set to the pipeline_run_events rowid, enabling automatic
 * replay of missed events on reconnect. Uses DB polling (2s interval) as the
 * delivery mechanism; events are persisted to pipeline_run_events before being
 * observed, so reconnects reliably replay missed events from DB.
 *
 * If the client cursor is older than the oldest retained event (gap detected),
 * a `resync` event is sent before the snapshot so the client can reset state.
 */
pub async fn stream_pipeline_run(
    ...
    let stream = async_stream::stream! {
        // Determine latest and oldest event rowids for gap detection
        let latest_rowid: Option<i64> = sqlx::query_scalar::<_, i64>(
        ...
        let snapshot_seq = latest_rowid.unwrap_or(0);

        // Detect both kinds of gaps:
        // 1. Cursor is older than our oldest retained event (events lost to pruning/wraparound)
        // 2. Cursor is significantly newer than latest (DB reset / run recreated)
        let mut need_resync = false;
        if initial_cursor > 0 {
            let oldest_rowid: Option<i64> = sqlx::query_scalar::<_, i64>(
                "SELECT COALESCE(MIN(rowid), 0) FROM pipeline_run_events WHERE run_id = ?"
            )
            .bind(&run_id)
            .fetch_optional(&db)
            .await
            ...
            if let Some(oldest) = oldest_rowid {
                if oldest > initial_cursor + 1 {
                    need_resync = true;
                }
            }
            // Client well ahead of server (DB reset / run recreated with lower rowids)
            if initial_cursor > snapshot_seq + 100 {
                need_resync = true;
            }
        }
        ...
        if need_resync {
            yield Ok::<_, Infallible>(Event::default()
                ...
                .data(serde_json::json!({
                    ...
                    "message": "Pipeline 事件游标已过期，请重新加载运行状态",
                    "cursor": snapshot_seq,
                }).to_string()));
        }

        // Send initial snapshot (authoritative state as of snapshot_seq)
        ...
        // Immediately replay events the client missed (rowid > initial_cursor up to snapshot_seq)
        // so we don't wait for the 2s poll interval to catch up.
        // If resync was requested, start from 0 (full catch-up from snapshot forward).
        let replay_start = if need_resync { snapshot_seq } else { initial_cursor };
        if snapshot_seq > replay_start {
            let missed: Vec<(String, String, String, i64)> = match sqlx::query_as::<_, (String, String, String, i64)>(
                "SELECT event_type, payload_json, source, rowid
                 FROM pipeline_run_events WHERE run_id = ? AND rowid > ? AND rowid <= ?
            ...
            .await {
                Ok(events) => events,
                Err(e) => {
                    tracing::warn!("[Pipeline-SSE] Initial replay query failed for run {}: {}", run_id, e);
                    Vec::new()
                }
            };
            for (event_type, payload_json, source, rowid) in missed {
                let payload: serde_json::Value = serde_json::from_str(&payload_json)
                    .unwrap_or_else(|_| serde_json::Value::String(payload_json.clone()));
                let wrapper = PipelineEventPayload {
                    event_type: event_type.clone(),
                    payload,
                    source,
                    seq: rowid,
                };
                let data = serde_json::to_string(&wrapper).unwrap_or_default();
                yield Ok(Event::default()
                    .event(&event_type)
                    .id(rowid.to_string())
                    .data(data));
        ...
        // Continue polling for new events after snapshot_seq
        let mut last_event_id: i64 = snapshot_seq;
        ...
        loop {
            ...
            .await {
                Ok(events) => events,
                Err(e) => {
                    tracing::warn!("[Pipeline-SSE] Poll error for run {}: {}", run_id, e);
                    empty_polls += 1;
                    if empty_polls > 10 {
                        // Give up after sustained DB errors
                        break;
                    }
                    continue;
                }
            ...
            if events.is_empty() {
                ...
                .fetch_optional(&db)
                .await {
                    if matches!(status.as_str(), "completed" | "failed" | "cancelled") && empty_polls > 3 {
                        // Run is done and no new events for a few polls — send done
                        yield Ok(Event::default()
                            .event("done")
                ...
                if empty_polls > 900 {
                    yield Ok(Event::default()
                        .event("done")
                        .id(last_event_id.to_string())
                        .data(r#"{"reason":"timeout"}"#.to_string()));
                    break;
                }
                continue;
            }
            ...
            for (event_type, payload_json, source, rowid) in events {
                last_event_id = last_event_id.max(rowid);
                // Parse payload as JSON (fallback to raw string if invalid JSON)
                let payload: serde_json::Value = serde_json::from_str(&payload_json)
                    .unwrap_or_else(|_| serde_json::Value::String(payload_json.clone()));
                let wrapper = PipelineEventPayload {
                    event_type: event_type.clone(),
