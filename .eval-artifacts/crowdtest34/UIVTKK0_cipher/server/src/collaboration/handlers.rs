...
/// 协同事件 SSE 流
///
/// 支持 Last-Event-ID 断点续传（受 broadcaster buffer 限制）；
/// 若发生 Lagged（服务端事件积压溢出），发送 resync 信号让客户端重新拉取状态。
pub async fn stream_collaboration_events(
    ...
    // Read Last-Event-ID from header. Collaboration events use a per-connection
    // monotonic counter; if cursor is provided and buffer has advanced beyond it,
    // we send a resync signal.
    ...
    let stream = async_stream::stream! {
        // Per-connection event counter. We increment for every event sent to this client.
        let mut event_counter: u64 = 0;

        // If client provided a cursor we cannot honor (broadcast buffer is bounded
        // and doesn't support arbitrary replay), send a resync and let client reload.
        if last_event_id.is_some() {
            // We can't replay old broadcast events; signal resync.
            // Client should reload active session state via REST API.
            let resync = serde_json::json!({
                "reason": "cursor_unavailable",
                "message": "协作事件游标已过期，请刷新状态"
            });
        ...
        loop {
            match rx.recv().await {
                ...
                Err(tokio::sync::broadcast::error::RecvError::Lagged(skipped)) => {
                    // Events were dropped because client was too slow; signal resync.
                    event_counter = event_counter.saturating_add(1);
                    let resync = serde_json::json!({
                        "reason": "buffer_overflow",
                        "message": format!("协作事件溢出，丢失 {} 条事件，请刷新状态", skipped)
                    });
                    yield Ok(Event::default()
                        .event("resync")
                        .id(event_counter.to_string())
