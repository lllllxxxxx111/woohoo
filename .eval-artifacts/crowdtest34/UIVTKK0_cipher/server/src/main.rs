...
pub struct AppState {
    ...
    pub sse_event_log: sse_event_log::SseEventLog,
    pub started_at: i64,
}
...
async fn main() {
    ...
    // 创建 SSE 事件日志（用于游标重放和断连恢复）
    let sse_event_log = sse_event_log::SseEventLog::new(Some(pool.clone()));
    sse_event_log.init_from_db().await;

    // 创建 AI 客户端
    let ai_client = AiClient::new();
    let mut ai_runtime =
        ai::runtime::AiTaskRuntime::new(config.ai_max_concurrent_tasks, Some(pool.clone()));
    ai_runtime.set_event_log(sse_event_log.clone());
