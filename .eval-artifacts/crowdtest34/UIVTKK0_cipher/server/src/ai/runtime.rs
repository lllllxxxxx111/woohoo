...
use crate::sse_event_log::{LoggedEvent, SseEventLog};
...
impl AiTaskRuntime {
    ...
    /// Attach the SSE event log for sequence number assignment.
    /// Must be called after construction and before events are published.
    pub fn set_event_log(&mut self, event_log: SseEventLog) {
    ...
    pub async fn create_task(&self, user_id: String, mut task: AiTask) -> AiTask {
        ...
        let event = AiTaskEvent {
            event_seq: 0, // assigned during publish
    ...
    pub async fn mark_completed(
        ...
        self.update_task(
            ...
            task_id,
            |task| {
                if matches!(task.status, AiTaskStatus::Running | AiTaskStatus::Queued) {
    ...
    /// Mark a task as blocked (waiting for prerequisite/manual review).
    ...
    /// Access the event log (for SSE handlers that need to assign seq numbers).
    pub fn event_log(&self) -> Option<&SseEventLog> {
    ...
    fn publish(&self, user_id: String, mut event: AiTaskEvent) {
        // Assign sequence number atomically with buffer insertion so concurrent
        // publish() calls cannot interleave and produce out-of-order seqs.
        // publish() is synchronous (no .await), so holding std::sync::Mutex across
        // short serde serialization is acceptable and avoids a reordering race.
        let payload: String = if let Some(ref log) = self.event_log {
            let mut buf = log.buffer.lock().unwrap();
            let seq = buf.reserve_seq();
            event.event_seq = seq;
            ...
            buf.push_with_seq(LoggedEvent {
                seq,
                user_id: user_id.clone(),
                stream: "ai_task".to_string(),
                stream_key: event.task.id.clone(),
                event_type: event.event_type.clone(),
                payload: payload.clone(),
                created_at: now,
            });
            // Drop lock before fire-and-forget persist (spawns a task that doesn't need the lock)
            drop(buf);
            log.persist(seq, &user_id, "ai_task", &event.task.id, &event.event_type, &payload, now);
            payload
        } else {
...
pub fn is_terminal(status: &AiTaskStatus) -> bool {
    matches!(
        status,
...
/// Returns the state ordering for out-of-order protection.
/// Higher number = later in lifecycle; events with lower order must not
/// overwrite states with higher order.
pub fn state_order(status: &AiTaskStatus) -> u8 {
    match status {
        AiTaskStatus::Queued => 1,
        AiTaskStatus::Running => 2,
        AiTaskStatus::Blocked => 3,
        AiTaskStatus::Completed | AiTaskStatus::Failed | AiTaskStatus::Cancelled => 4,
    }
}
