...
pub struct AiTaskEvent {
    /// Monotonic sequence number assigned by the event buffer for cursor replay.
    /// Present on all events emitted after the 024 migration; 0 for legacy in-memory only events.
