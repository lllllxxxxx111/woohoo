...
pub async fn stream_collaboration_events(
    ...
    let stream = async_stream::stream! {
        // Send initial snapshot with current seq as cursor anchor
        let current_seq = broadcaster.current_seq();
        yield Ok(
            Event::default()
        ...
        loop {
            match rx.recv().await {
                ...
                Err(tokio::sync::broadcast::error::RecvError::Lagged(skipped)) => {
                    // Signal resync rather than silently dropping events
                    yield Ok(
                        Event::default()
                            .event("resync")
                            .data(serde_json::to_string(&serde_json::json!({
