...
pub struct AiTask {
    ...
    /// Sequence number of the last event for this task (for cursor ordering).
    /// Not persisted; populated at runtime from event seq.
...
pub struct AiTaskEvent {
    /// Monotonically increasing sequence number across all events for this runtime.
    /// Used as SSE `id:` field for Last-Event-ID replay and deduplication.
    pub seq: i64,
...
pub struct AiTaskFilter {
    ...
    /// Optional cursor for replay: only return events with seq > cursor.
    #[serde(default)]
