...
/// Maximum number of events retained in the in-memory ring buffer for replay.
/// After this many events, older events are evicted. If a client's cursor falls
/// behind the oldest retained event, the server sends a `resync` signal instead
...
/// Result of a replay request: either the events to replay, or a signal
/// that the cursor is too old and the client must resync from snapshot.
#[derive(Debug)]
pub enum ReplayResult {
    /// Events with seq > cursor, in order.
    Events(Vec<AiTaskEvent>),
    /// Cursor is behind the buffer window; client must do a full resync.
    ResyncNeeded { oldest_available: i64 },
}
...
pub struct AiTaskRuntime {
    ...
    /// Global monotonically increasing event sequence counter.
    event_seq: Arc<AtomicI64>,
    /// Ring buffer of recent events for cursor-based replay.
    /// Stores (seq, AiTaskEvent) pairs ordered by seq.
    event_buffer: Arc<RwLock<VecDeque<(i64, String, AiTaskEvent)>>>,
}
...
impl AiTaskRuntime {
    pub fn new(max_concurrent_tasks: usize, db: Option<SqlitePool>) -> Self {
        ...
        Self {
            ...
            event_buffer: Arc::new(RwLock::new(VecDeque::with_capacity(EVENT_BUFFER_SIZE))),
        }
    }
    ...
    pub async fn mark_completed(
        ...
        self.update_task(
            ...
            task_id,
            |task| {
                // Allow completion from Queued (very fast tasks) or Running
    ...
    pub async fn global_snapshot(&self) -> RuntimeTaskSnapshot {
        ...
        for stored in tasks.values() {
            match stored.task.status {
                ...
                AiTaskStatus::Cancelled => snapshot.cancelled_tasks += 1,
            }
        }
    ...
    pub async fn project_task_counts(
        ...
        for stored in tasks.values().filter(|stored| stored.user_id == user_id) {
            ...
            match stored.task.status {
                ...
                AiTaskStatus::Cancelled => entry.cancelled += 1,
            }
        }
        ...
        F: FnOnce(&mut AiTask),
    {
        ...
        self.publish(user_id.to_string(), event).await;
        Some(task)
    }
    ...
    pub async fn emit_content_delta(
        ...
        // Need write lock to update the task's seq tracking
        let mut tasks = self.tasks.write().await;
        let stored = tasks.get_mut(task_id)?;
        if stored.user_id != user_id {
            return None;
    ...
    async fn publish(&self, user_id: String, event: AiTaskEvent) {
        let seq = event.seq;

        // Store in ring buffer for replay (direct write, preserves order)
        {
            let mut buf = self.event_buffer.write().await;
            buf.push_back((seq, user_id.clone(), event.clone()));
    ...
    /// Replay events for a user that occurred after the given cursor (seq).
    /// Returns either the matching events or a signal that resync is needed.
    pub async fn replay_events(&self, user_id: &str, after_seq: i64) -> ReplayResult {
        let buf = self.event_buffer.read().await;
        if buf.is_empty() {
            return ReplayResult::Events(Vec::new());
        }

        let oldest_seq = buf.front().map(|(s, _, _)| *s).unwrap_or(0);
        if after_seq < oldest_seq - 1 {
            return ReplayResult::ResyncNeeded {
                oldest_available: oldest_seq,
            };
        }

        let events: Vec<AiTaskEvent> = buf
            .iter()
            .filter(|(seq, uid, _)| *seq > after_seq && uid == user_id)
    ...
    /// Return the current max sequence number (useful for initial snapshot cursor).
    pub fn current_seq(&self) -> i64 {
        self.event_seq.load(Ordering::Relaxed) - 1
    }
}
