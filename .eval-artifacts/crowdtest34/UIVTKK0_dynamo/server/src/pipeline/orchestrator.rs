...
async fn load_task_snapshot(
    ...
    Ok(persisted.and_then(|task| {
        // Status in DB is stored as JSON-serialized string (e.g., '"completed"').
        // Parse it via serde_json to get the plain status name. Fall back to
        // treating the raw string as-is for legacy non-JSON rows.
        let status: String = serde_json::from_str(&task.status)
            .unwrap_or_else(|_| task.status.trim_matches('"').to_string());
        // Validate it's a known terminal status; otherwise treat as missing.
        match status.as_str() {
            "completed" | "failed" | "cancelled" | "queued" | "running" => Some(TaskSnapshot {
                status,
                result: task.result,
                error: task.error,
            }),
