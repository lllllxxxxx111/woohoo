 ...
 *
 * Supports cursor-based replay via Last-Event-ID header or ?cursor= query param.
 * All events include `id:` set to the pipeline_run_events rowid for reliable replay.
 * If cursor is invalid/expired, sends a `resync` event with guidance.
 */
pub async fn stream_pipeline_run(
    ...
    // Determine cursor from query param or Last-Event-ID header
    let initial_cursor: Option<i64> = params
        .get("cursor")
    ...
    let stream = async_stream::stream! {
        // Fetch the current max event id for this run to anchor the snapshot
        ...
        let client_cursor = initial_cursor;
        let max_id = max_rowid.unwrap_or(0);
        let has_client_cursor = client_cursor.is_some();

        // If client provided a cursor that's way ahead of known events,
        // this likely indicates a server restart where rowids reset.
        // Send resync rather than silently missing events.
            ...
            if cursor > max_id + 10 {
                yield Ok::<_, Infallible>(Event::default()
                    ...
                    .data(serde_json::to_string(&serde_json::json!({
                        ...
                        "oldestAvailable": 0,
                        "cursor": max_id,
                        "message": "事件游标无效，可能是服务重启导致，请从快照重新同步"
                    })).unwrap_or_default()));
                // Send snapshot after resync
                yield Ok::<_, Infallible>(Event::default()
                    .id(max_id.to_string())
                    .event("snapshot")
                    .data(serde_json::to_string(&serde_json::json!({
                        "status": "connected",
                        "cursor": max_id
                    })).unwrap_or_else(|_| r#"{"status":"connected"}"#.to_string())));
                // Skip replay; let client reconcile from snapshot
                let mut last_event_id: i64 = max_id;
                ...
                loop {
                    ...
                    .await {
                        Ok(events) => events,
                        Err(e) => {
                            tracing::debug!("Pipeline stream poll error: {}", e);
                            continue;
                        }
                    };
        ...
        // Replay events the client missed since its cursor (reconnect only).
        // For initial connections (no cursor), skip historical replay — the
        // snapshot represents current state and replaying all history causes
        // duplicate state transitions on the client.
        let mut after_cursor = client_cursor.unwrap_or(max_id);
        if has_client_cursor && after_cursor < max_id {
            let missed_events: Vec<(String, String, String, i64)> = match sqlx::query_as::<_, (String, String, String, i64)>(
                "SELECT event_type, payload_json, source, rowid
                 FROM pipeline_run_events WHERE run_id = ? AND rowid > ?
            ...
            .await {
                Ok(events) => events,
                Err(e) => {
                    tracing::debug!("Pipeline replay error: {}", e);
                    Vec::new()
                }
            };

            for (event_type, payload, source, rowid) in missed_events {
                after_cursor = after_cursor.max(rowid);
                let data = format!(r#"{{"type":"{}","payload":{},"source":"{}"}}"#,
                    event_type,
                    payload.as_str(),
                    source
        ...
        // Send snapshot AFTER replay so event IDs are monotonically increasing.
        // The snapshot cursor is the highest known event ID so far.
        let snapshot_cursor = after_cursor.max(max_id);
        yield Ok::<_, Infallible>(Event::default()
