...
export function createUsageTaskPipelineApi(requestApi: RequestApi) {
   ...
   * event IDs for cursor-based replay, and [DONE]/done termination.
   */
  const streamPipelineRun = (
    ...
    void (async () => {
      try {
        ...
        // Import SSE consumer dynamically to avoid circular deps
        const { createSseConsumer, isDoneSignal } = await import('./sseConsumer');
