...
use super::{
    ...
    router::{self, RoutingCapability, RoutingOperation, RoutingRequest},
...
struct ResolvedChatContext {
    ...
    /**
     * Original explicit endpoint ID from the request (if user specified one).
     * Used by the router to honor explicit endpoint selection without silent fallback.
     */
    explicit_endpoint_id: Option<String>,
    /**
     * Whether assistant actions/tool use are allowed for this request.
     * Affects routing to ensure endpoints support tool calling.
     */
    allow_assistant_actions: bool,
...
pub async fn ai_chat(
    ...
    let routing_req = RoutingRequest {
        ...
        context_tokens: Some(prepared.input_chars / 4), // rough token estimate
        requires_streaming: matches!(context.stream_fallback_mode, StreamFallbackMode::Force),
        requires_tools: req.allow_assistant_actions,
        request_id,
        task_id: None,
        run_id: None,
        step_id: None,
    ...
    let routed_result = router::routed_chat_call(
        &state.db,
        &state.ai_client,
        &routing_req,
    ...
    let (routed, fallback_info) = match routed_result {
        Ok(r) => {
            // Update context with the actual endpoint/model used
            if let Ok(ep) = sqlx::query_as::<_, AiEndpoint>(
            ...
            (
                ...
                if r.was_fallback {
                    Some((r.fallback_reason, r.attempt_count))
                } else {
                    None
                },
            )
    ...
    if let Some((reason, attempts)) = fallback_info {
        response_json["routing"] = serde_json::json!({
            "wasFallback": true,
...
pub async fn ai_chat_stream(
    ...
    // Build routing request for intelligent endpoint selection
    let stream_routing_req = RoutingRequest {
        user_id: user_id.0.clone(),
        capability: RoutingCapability::Chat,
        operation: RoutingOperation::Stream,
        explicit_endpoint_id: req.endpoint_id.clone(),
        requested_model: req.model.clone(),
        context_tokens: Some(prepared.input_chars / 4),
    ...
    let routed_stream = router::routed_chat_stream_call(
        &state.db,
        &state.ai_client,
        &stream_routing_req,
        prepared.messages,
    ...
    let (ai_stream, routed_endpoint_id, routed_provider, routed_model, api_key) = match routed_stream {
        Ok(r) => {
            // Load the full endpoint (including api_key) for usage recording
            let api_key = match sqlx::query_as::<_, AiEndpoint>(
                "SELECT * FROM ai_endpoints WHERE id = ? AND user_id = ?",
            )
            .bind(&r.endpoint_id)
            .bind(&user_id.0)
            ...
            (r.stream, r.endpoint_id, r.provider, r.model, api_key)
        }
        Err(error) => {
            if let Err(message_error) =
                save_failure_message(&state.db, &context, &error.to_string(), None).await
    ...
    let endpoint_id = routed_endpoint_id;
    let provider = routed_provider;
    let model_for_save = routed_model;
...
async fn run_ai_task(
    ...
    // Build routing request for intelligent endpoint selection with fallback
    let task_routing_req = RoutingRequest {
        user_id: user_id.clone(),
        capability: RoutingCapability::Chat,
        operation: RoutingOperation::Task,
        explicit_endpoint_id: context.explicit_endpoint_id.clone(),
        requested_model: Some(context.model.clone()),
        context_tokens: Some(prepared.input_chars / 4),
        requires_streaming: true,
        requires_tools: context.allow_assistant_actions,
    ...
    let routed_stream_result = router::routed_chat_stream_call(
        &state.db,
        &state.ai_client,
        &task_routing_req,
        prepared.messages.clone(),
    ...
    match routed_stream_result {
        Ok(routed) => {
            // Update context with the actual endpoint/model selected by router
            if let Ok(ep) = sqlx::query_as::<_, AiEndpoint>(
                "SELECT * FROM ai_endpoints WHERE id = ? AND user_id = ?",
            )
            .bind(&routed.endpoint_id)
            .bind(&user_id)
            .fetch_one(&state.db)
            .await
            {
            ...
            context.model = routed.model.clone();

            let mut ai_stream = routed.stream;
            let mut full_content = String::new();
            let mut visible_content = String::new();
            let mut persisted_visible_content = String::new();
            let mut last_stream_persist_at = Instant::now();
        ...
        Err(error) => {
            tracing::warn!(
                task_id = %task_id,
                error = %error,
                "Failed to start routed task stream, falling back to non-stream chat"
            );
            run_ai_task_non_stream(
                &state,
                &user_id,
...
async fn run_ai_task_non_stream(
    ...
    user_id: &str,
    task_id: &str,
    context: &mut ResolvedChatContext,
    ...
    match routed_result {
        Ok(routed) => {
            ...
            // Record routing fallback info for frontend visibility
            if routed.was_fallback {
                let _ = state
                    .ai_runtime
                    .set_routing_info(
                        user_id,
                        task_id,
                        true,
                        routed.attempt_count,
                        routed.fallback_reason.clone(),
                    )
                    .await;
            }
