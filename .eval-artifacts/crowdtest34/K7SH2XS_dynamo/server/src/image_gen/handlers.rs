...
pub async fn create_generation(
    ...
    let task_n = req.n;
    let task_endpoint_id = resolved.endpoint.id.clone();
    let task_requested_model = req.model.clone();
    let task_explicit_endpoint_id = req.endpoint_id.clone();
...
async fn run_generation_task(
    ...
    let routing_req = RoutingRequest {
        ...
        capability: RoutingCapability::ImageGeneration,
        operation: RoutingOperation::ImageGeneration,
        ...
        async move {
            ...
            .await
            {
                ...
                Ok(Err(e)) => {
                    ...
                    let http_status = extract_http_status_from_err(&err_str);
                    Err((http_status, err_str))
                }
                Err(_) => Err((Some(408), "image generation timed out".to_string())),
            }
        }
    })
    .await
    ...
    let response = match outcome.result {
        ...
        None => {
            let last_error = outcome
                ...
                .unwrap_or_else(|| "all image generation endpoints failed".to_string());
...
fn extract_http_status_from_err(error: &str) -> Option<u16> {
