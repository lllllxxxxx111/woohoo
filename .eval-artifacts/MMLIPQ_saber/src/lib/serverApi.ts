...
export async function getExportPreflight(projectId: string): Promise<ServerPreflightResult> {
  return requestApi<ServerPreflightResult>(`/api/projects/${projectId}/export/preflight`);
}

export async function recordExport(
  projectId: string,
  data: RecordExportRequest,
): Promise<ExportAuditRecord> {
  return requestApi<ExportAuditRecord>(`/api/projects/${projectId}/export`, {
    method: 'POST',
    body: JSON.stringify(data),
  });
}

export async function listProjectExports(projectId: string, limit = 20): Promise<ExportAuditRecord[]> {
  return requestApi<ExportAuditRecord[]>(`/api/projects/${projectId}/exports?limit=${limit}`);
}

export async function listMyExports(limit = 20): Promise<ExportAuditRecord[]> {
  return requestApi<ExportAuditRecord[]>(`/api/exports?limit=${limit}`);
}

export async function getExportRecord(id: string): Promise<ExportAuditRecord> {
  return requestApi<ExportAuditRecord>(`/api/exports/${id}`);
}
