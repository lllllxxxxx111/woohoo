...
import { Asset, AssetReferenceSummary } from '../../../../types';
import { useAppActions } from '../../../../context/useAppActions';
import { useToast } from '../../../../context/useToast';
import { getServerAssetBlob, getStoredServerProfile, searchServerAssets, getAssetReferences, deleteServerAsset } from '../../../../lib/serverApi';
...
const DeleteConfirmDialog: React.FC<{
  asset: Asset;
  references: AssetReferenceSummary | null;
  loading: boolean;
  onConfirm: (force: boolean) => void;
  onCancel: () => void;
}> = ({ asset, references, loading, onConfirm, onCancel }) => {
  if (loading) {
    return (
      <div className={styles.deleteConfirmOverlay} onClick={onCancel}>
        <div className={styles.deleteConfirmDialog} onClick={(e) => e.stopPropagation()}>
          <Loader2 size={24} className={styles.previewSpinner} />
          <p>正在检查引用关系...</p>
        </div>
      </div>
    );
  ...
  const hasRefs = references && references.hasReferences && references.totalCount > 0;

  return (
    <div className={styles.deleteConfirmOverlay} onClick={onCancel}>
      <div className={styles.deleteConfirmDialog} onClick={(e) => e.stopPropagation()}>
        <div className={styles.deleteConfirmHeader}>
          <AlertTriangle size={20} className={hasRefs ? styles.warningIcon : styles.infoIcon} />
          <h3>删除素材</h3>
        </div>
        <p>
          确定要删除 <strong>{asset.name}</strong> 吗？
        </p>
        {hasRefs && references && (
          <div className={styles.deleteConfirmReferences}>
            <p className={styles.deleteConfirmRefWarning}>
              该素材被 <strong>{references.totalCount}</strong> 个位置引用：
            </p>
            <ul className={styles.deleteConfirmRefList}>
              {references.references.slice(0, 5).map((ref, idx) => (
                <li key={`ref-${idx}`}>
                  <span className={styles.deleteConfirmRefType}>
                    {ref.refType === 'storyboard' ? '分镜' : '流水线'}
                  </span>
                  <span className={styles.deleteConfirmRefProject}>{ref.projectName}</span>
                  <span className={styles.deleteConfirmRefTitle}>{ref.refTitle}</span>
                </li>
              ))}
              {references.references.length > 5 && (
                <li className={styles.deleteConfirmRefMore}>
                  还有 {references.references.length - 5} 个引用...
                </li>
              )}
            </ul>
            <p className={styles.deleteConfirmForceHint}>
              删除将导致这些引用失效。确认后将强制删除。
            </p>
          </div>
        )}
        <div className={styles.deleteConfirmActions}>
          <button type="button" className={styles.deleteCancelBtn} onClick={onCancel}>
            取消
          </button>
          <button
            type="button"
            className={hasRefs ? styles.deleteForceBtn : styles.deleteConfirmBtn}
            onClick={() => onConfirm(hasRefs === true)}
          >
            {hasRefs ? '强制删除' : '确认删除'}
          </button>
        </div>
...
export const AssetLibrary: React.FC = () => {
  ...
  const { uploadAssets, updateAsset, refreshWorkspace } = useAppActions();
  ...
  const activeTag = assetLibraryView.tag;
  ...
  const [deleteConfirmReferences, setDeleteConfirmReferences] = useState<AssetReferenceSummary | null>(null);
  const [isCheckingReferences, setIsCheckingReferences] = useState(false);

  // 引用信息状态
  const [assetReferences, setAssetReferences] = useState<AssetReferenceSummary | null>(null);
  ...
  useEffect(() => {
    ...
    getAssetReferences(selectedAsset)
      .then((refs) => {
        if (!cancelled) {
          setAssetReferences(refs);
        }
      })
      .catch(() => {
        if (!cancelled) {
  ...
  const selectedAssetTags = useMemo(
    () => (selectedAssetData ? getAssetTags(selectedAssetData) : []),
    [selectedAssetData],
  );
  ...
  const selectedAssetInfoRows = useMemo<DetailInfoRow[]>(() => {
    ...
    return [
      ...
      {
        label: '标签',
        value: selectedAssetTags.length > 0 ? selectedAssetTags.join(', ') : '无',
      },
      {
        label: '引用数',
        value: assetReferences ? `${assetReferences.totalCount} 个` : isLoadingReferences ? '加载中...' : '未知',
      },
    ];
  }, [
  ...
  useEffect(() => {
    ...
    const needsServerSearch =
      libraryScope === 'all' && (searchQuery.trim() !== '' || activeTag !== null);
    ...
    searchTimeoutRef.current = setTimeout(() => {
      searchServerAssets({
        ...
        assetType: filterType !== 'all' ? (filterType as Asset['type']) : undefined,
  ...
  }, [searchQuery, libraryScope, activeTag, filterType, favoriteOnly, ratingFilter, activeState.projectId]);
  ...
  // 所有可用标签
  const allAvailableTags = useMemo(() => extractAllTags(assets), [assets]);
  ...
  // 有效素材列表：服务器搜索结果 或 本地过滤
  const effectiveAssets = useMemo(() => {
    if (libraryScope === 'all' && serverSearchResults !== null) {
      return serverSearchResults;
    }
    return scopedAssets;
  }, [libraryScope, scopedAssets, serverSearchResults]);

  const activeProjectName = activeState.projectId
    ? projectNameById.get(activeState.projectId) || '当前项目'
    : '未选择项目';

  // 本地过滤（标签、类型、收藏、评分、文本搜索）
  const filteredAssets = useMemo(() => {
    // 如果是跨项目搜索且有服务器结果，服务器已经处理了过滤
    if (libraryScope === 'all' && serverSearchResults !== null) {
    ...
    return effectiveAssets.filter((asset) => {
      ...
      const matchesTag = !activeTag || assetHasTag(asset, activeTag);
      const projectName = projectNameById.get(asset.projectId) || '';
      const matchesSearch =
        normalizedSearch === '' ||
        asset.name.toLowerCase().includes(normalizedSearch) ||
        projectName.toLowerCase().includes(normalizedSearch) ||
        // 搜索 metadata 中的 prompt、summary、description、tags
        getMetadataText(normalizeAssetMetadata(asset.metadata), 'prompt').toLowerCase().includes(normalizedSearch) ||
        getMetadataText(normalizeAssetMetadata(asset.metadata), 'summary').toLowerCase().includes(normalizedSearch) ||
        getMetadataText(normalizeAssetMetadata(asset.metadata), 'description').toLowerCase().includes(normalizedSearch);
      return matchesType && matchesFavorite && matchesRating && matchesTag && matchesSearch;
    });
  }, [activeTag, effectiveAssets, favoriteOnly, filterType, libraryScope, projectNameById, ratingFilter, searchQuery, serverSearchResults]);
  ...
  // 新的删除流程：先检查引用，再确认
  const handleAssetDeleteInitiate = (asset: Asset, event: React.MouseEvent) => {
    event.stopPropagation();
    setDeleteConfirmAsset(asset);
    setDeleteConfirmReferences(null);
    setIsCheckingReferences(true);

    getAssetReferences(asset.id)
      .then((refs) => {
        setDeleteConfirmReferences(refs);
        setIsCheckingReferences(false);
      })
      .catch(() => {
        // 引用检查失败时，允许正常删除（不阻塞）
        setDeleteConfirmReferences({
          assetId: asset.id,
          hasReferences: false,
          totalCount: 0,
          references: [],
  ...
  const handleDeleteConfirm = async (force: boolean) => {
    ...
    try {
      await deleteServerAsset(asset.id, force);
      showToast({
        type: 'success',
        title: '资产已删除',
  ...
  const handleTagToggle = (tag: string) => {
    if (activeTag === tag) {
      updateLibraryView({ tag: null });
    } else {
      updateLibraryView({ tag });
    }
    ...
    return (
      ...
      >
          ...
          {selectedAsset === asset.id && (
            <div className={styles.assetOverlay}>
              ...
              <button
                ...
                title="删除"
                onClick={(event) => void handleAssetDeleteInitiate(asset, event)}
              >
                <Trash2 size={16} />
              </button>
            </div>
        ...
        <div className={styles.assetInfo}>
          ...
          {tags.length > 0 && (
            <div className={styles.assetTagRow}>
              ...
              {tags.length > 3 && (
                <span className={styles.assetTagMore}>+{tags.length - 3}</span>
              )}
            </div>
          )}
          <div
    ...
    <div className={styles.container}>
      <div className={styles.topToolbar}>
        <div className={styles.searchBox}>
          ...
          <input
            type="text"
            placeholder={libraryScope === 'all' ? '跨项目搜索素材...' : '搜索当前项目素材...'}
            value={searchQuery}
        ...
        <div className={styles.actions}>
          ...
          {allAvailableTags.length > 0 && (
            <div className={styles.filterDropdown}>
              <button
                className={`${styles.toolBtn} ${activeTag ? styles.activeToolBtn : ''}`}
                onClick={() => {
              ...
              {showTagDropdown && (
                <div className={styles.filterMenu}>
                  <button
                    className={`${styles.filterItem} ${!activeTag ? styles.active : ''}`}
                  ...
                  {allAvailableTags.map((tag) => (
                    <button
                      key={tag}
                      className={`${styles.filterItem} ${activeTag === tag ? styles.active : ''}`}
            ...
            <div className={styles.detailToolbar}>
              ...
              <div className={styles.detailToolbarActions}>
                ...
                <button
                  ...
                  onClick={(event) => void handleAssetDeleteInitiate(selectedAssetData, event)}
                  title="删除"
                >
                  <Trash2 size={16} />
                  <span>删除</span>
                ...
                <div className={styles.detailInfoContent}>
                  ...
                  {selectedAssetTags.length > 0 && (
                    <section className={styles.detailTagsBlock}>
                      <div className={styles.detailTagsHeader}>
                        <Tag size={14} />
                        <span>标签</span>
                      </div>
                      <div className={styles.detailTagsList}>
                  ...
                  {/* 引用关系区域 */}
                  <section className={styles.detailRefBlock}>
                    <div className={styles.detailRefHeader}>
                      <Link size={14} />
                      <span>引用关系</span>
                    </div>
                    {isLoadingReferences ? (
                      <div className={styles.detailRefLoading}>
                        <Loader2 size={16} className={styles.previewSpinner} />
                        <span>加载引用关系...</span>
                      </div>
                    ) : assetReferences && assetReferences.hasReferences ? (
                      <div>
                        <p className={styles.detailRefSummary}>
                          被 {assetReferences.totalCount} 个位置引用
                        </p>
                        <ul className={styles.detailRefList}>
                          {assetReferences.references.map((ref, idx) => (
                            <li key={`ref-${idx}`} className={styles.detailRefItem}>
                              <span className={`${styles.detailRefType} ${ref.refType === 'storyboard' ? styles.refTypeStoryboard : styles.refTypePipeline}`}>
                                {ref.refType === 'storyboard' ? '分镜' : '流水线'}
                              </span>
                              <span className={styles.detailRefProject}>{ref.projectName}</span>
                              <span className={styles.detailRefTitle}>{ref.refTitle}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
                    ) : (
                      <p className={styles.detailRefEmpty}>暂无引用</p>
                    )}
                  </section>
