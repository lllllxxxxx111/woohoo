...
export async function searchServerAssets(params: AssetSearchParams) {
  ...
  const path = queryString
    ? `/api/assets/search?${queryString}`
    : '/api/assets/search';

  const serverAssets = await requestApi<ServerAsset[]>(path);
  return serverAssets.map(mapAsset);
}

export async function getAssetReferences(assetId: string) {
  return requestApi<AssetReferenceSummary>(`/api/assets/${assetId}/references`);
}
