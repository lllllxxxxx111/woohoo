...
use super::{
    model::{Asset, AssetSearchQuery, CreateAssetReq, DeleteAssetQuery, UpdateAssetReq},
    repo,
};
...
pub async fn update_asset(
    ...
    // 合并 metadata 而不是覆盖：保留已有字段，只更新传入的字段
    let metadata = if let Some(new_metadata) = req.metadata {
        ...
        if let serde_json::Value::Object(ref mut map) = merged {
            if let serde_json::Value::Object(new_map) = new_metadata {
...
pub async fn delete_asset(
    ...
    if !params.force.unwrap_or(false) {
        let ref_count = repo::count_asset_references(&state.db, &id).await?;
        if ref_count > 0 {
            return Err(AppError::Conflict(format!(
                "该素材被 {} 个位置引用（分镜或流水线），删除将影响关联内容。请确认后使用 force=true 强制删除。",
                ref_count
            )));
        }
    }
...
pub async fn search_assets(
    ...
    let assets = repo::search_assets(&state.db, &user_id.0, &query).await?;
    Ok(Json(assets))
}
...
pub async fn get_asset_references(
    ...
    Extension(user_id): Extension<UserId>,
    Path(id): Path<String>,
) -> AppResult<Json<super::model::AssetReferenceSummary>> {
    // 先验证用户有该素材的访问权限
    ...
    Ok(Json(super::model::AssetReferenceSummary {
        asset_id: id,
