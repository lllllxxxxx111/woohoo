...
export async function deleteServerAsset(assetId: string, options?: { force?: boolean }) {
 ...
 * 跨项目搜索素材
 */
export async function searchServerAssets(params: AssetSearchParams): Promise<AssetSearchResponse> {
  ...
  if (params.ratingMin) searchParams.set('ratingMin', String(params.ratingMin));
  ...
  // Server returns AssetSearchItem with projectName; reuse ServerAsset shape + projectName
  type ServerSearchItem = ServerAsset & { projectName: string };
  ...
  const resp = await requestApi<ServerSearchResponse>(path);
  return {
    items: resp.items.map((item) => {
      // mapAsset expects ServerAsset; projectName is extra info we add after mapping
      ...
      const asset = mapAsset(serverAsset);
      return { ...asset, projectName: item.projectName };
    }),
    total: resp.total,
    offset: resp.offset,
    limit: resp.limit,
 ...
 * 查询素材引用关系
 */
export async function getServerAssetReferences(assetId: string): Promise<AssetReferencesResponse> {
