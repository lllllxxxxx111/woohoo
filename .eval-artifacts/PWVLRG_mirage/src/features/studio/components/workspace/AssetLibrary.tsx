...
import { Asset, type AssetReference } from '../../../../types';
...
export const AssetLibrary: React.FC = () => {
  ...
  const { uploadAssets, deleteAsset, updateAsset, refreshWorkspace, searchAssets, getAssetReferences } = useAppActions();
  ...
  // Cross-project search state
  const [remoteSearchResults, setRemoteSearchResults] = useState<Asset[]>([]);
  ...
  const [selectedAssetReferences, setSelectedAssetReferences] = useState<AssetReference[]>([]);
  ...
  // Collect all available tags from scoped assets
  const allAvailableTags = useMemo(() => collectAllTags(scopedAssets), [scopedAssets]);

  // Selected tag filter from assetLibraryView
  const selectedTagFilter = assetLibraryView.tag ?? null;
  const projectFilter = assetLibraryView.projectId ?? null;

  // Remote cross-project search (debounced)
  useEffect(() => {
    if (libraryScope !== 'all') {
    ...
    // For 'all' scope, we use the backend search when there are filters or search query
    const hasFilters =
      searchQuery.trim() !== '' ||
      filterType !== 'all' ||
      favoriteOnly ||
    ...
    if (!hasFilters) {
      // No filters - just use all local assets
      setRemoteSearchResults([]);
      setRemoteSearchTotal(0);
      return undefined;
    }
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setRemoteSearchResults(resp.items);
          setRemoteSearchTotal(resp.total);
        }
      } catch {
        if (!cancelled) {
          setRemoteSearchResults([]);
  ...
  const filteredAssets = useMemo(() => {
    // When in 'all' scope and we have active filters, use remote search results
    if (libraryScope === 'all') {
      const hasFilters =
        searchQuery.trim() !== '' ||
        filterType !== 'all' ||
      ...
      // No filters - show all local assets (with project filter applied if set)
      let base = assets;
      if (projectFilter) {
        base = base.filter((a) => a.projectId === projectFilter);
      }
      return filterAssetsLocally(base, {
    ...
    // 'current' scope - use local filtering
    return filterAssetsLocally(scopedAssets, {
      searchQuery: searchQuery.trim(),
      filterType,
      tag: selectedTagFilter,
      favoriteOnly,
  ...
  const handleAssetDelete = async (asset: Asset, event: React.MouseEvent) => {
    ...
    setDeleteReferences([]);
    try {
      const { references } = await getAssetReferences(asset.id);
      setDeleteReferences(references);
    } catch {
      // 如果查询引用失败，仍允许进入确认流程（后端会保护）
      setDeleteReferences([]);
    } finally {
  ...
  const handleConfirmDelete = async (force: boolean) => {
    ...
    setIsDeleting(true);
    try {
      await deleteAsset(deleteConfirmAsset.id, { force });
      showToast({
        type: 'success',
        title: '资产已删除',
        message: `${deleteConfirmAsset.name} 已从项目资产库移除。`,
      });
      setSelectedAsset((prev) => (prev === deleteConfirmAsset.id ? null : prev));
  ...
  const handleAddTag = (tag: string) => {
    ...
    if (!trimmed) return;
    const existing = getAssetTags(selectedAssetData);
    if (existing.includes(trimmed)) return;
    const nextTags = [...existing, trimmed];
    updateAssetMetadata(selectedAssetData, { tags: nextTags });
    setTagInputValue('');
  };
  ...
  // Load references when selecting an asset
  useEffect(() => {
    if (!selectedAsset) {
    ...
    void getAssetReferences(selectedAsset)
      .then(({ references }) => {
        if (!cancelled) setSelectedAssetReferences(references);
      })
      .catch(() => {
        if (!cancelled) setSelectedAssetReferences([]);
      })
      .finally(() => {
  ...
  }, [selectedAsset, getAssetReferences]);
          ...
          {getAssetTags(asset).length > 0 && (
            <div className={styles.assetTagsRow}>
              {getAssetTags(asset).slice(0, 3).map((tag) => (
                <span key={tag} className={styles.assetTagChip}>
                  {tag}
                </span>
              ))}
              {getAssetTags(asset).length > 3 && (
                <span className={styles.assetTagMore}>+{getAssetTags(asset).length - 3}</span>
              )}
            </div>
          )}
          <div
    ...
    <div className={styles.container}>
      <div className={styles.topToolbar}>
        ...
        <div className={styles.actions}>
          ...
          <div className={styles.filterDropdown}>
            <button
              className={`${styles.toolBtn} ${selectedTagFilter ? styles.activeToolBtn : ''}`}
              onClick={() => {
            ...
            >
              <Tag size={16} />
              {selectedTagFilter || '标签'}
            </button>
            {showTagDropdown && (
              <div className={styles.filterMenu}>
                <button
                  className={`${styles.filterItem} ${!selectedTagFilter ? styles.active : ''}`}
                  onClick={() => {
                    setAssetLibraryView({ tag: null });
                ...
                {allAvailableTags.length === 0 && (
                  <div className={styles.filterItem} style={{ opacity: 0.5, cursor: 'default' }}>
                    暂无标签
                  </div>
                )}
                {allAvailableTags.map((tag) => (
                  <button
                    key={tag}
                    className={`${styles.filterItem} ${selectedTagFilter === tag ? styles.active : ''}`}
                    onClick={() => {
                      setAssetLibraryView({ tag });
                      setShowTagDropdown(false);
                    }}
                  >
                    <Tag size={14} />
          ...
          {/* Project filter (only visible in 'all' scope) */}
          {libraryScope === 'all' && (
            <div className={styles.filterDropdown}>
              <button
                className={`${styles.toolBtn} ${projectFilter ? styles.activeToolBtn : ''}`}
                onClick={() => {
                  setShowProjectFilterDropdown((prev) => !prev);
                  setShowFilterDropdown(false);
                  setShowRatingDropdown(false);
                  setShowTagDropdown(false);
                }}
              ...
              {showProjectFilterDropdown && (
                <div className={styles.filterMenu}>
                  <button
                    ...
                    onClick={() => {
                      setAssetLibraryView({ projectId: null });
                      setShowProjectFilterDropdown(false);
                    }}
                  >
                    全部项目
                  ...
                  {projects.map((project) => (
                    <button
                      ...
                      onClick={() => {
                        setAssetLibraryView({ projectId: project.id });
                        setShowProjectFilterDropdown(false);
                      }}
                    >
                      <FolderOpen size={14} />
                ...
                <div className={styles.detailInfoContent}>
                  ...
                  <section className={styles.detailTagsBlock}>
                    ...
                    <div className={styles.detailTagsList}>
                      {getAssetTags(selectedAssetData).length === 0 && !editingTags && (
                        <span className={styles.detailMuted}>暂无标签</span>
                      )}
                      {getAssetTags(selectedAssetData).map((tag) => (
                      ...
                      {editingTags && (
                        <input
                          type="text"
                          className={styles.detailTagInput}
                          placeholder="输入标签后回车"
                          value={tagInputValue}
                          autoFocus
                  ...
                  <section className={styles.detailReferencesBlock}>
                    <div className={styles.detailReviewHeader}>
                      ...
                      {isLoadingReferences && <Loader2 size={14} className={styles.spinnerIcon} />}
                    </div>
                    {selectedAssetReferences.length === 0 && !isLoadingReferences && (
                      <p className={styles.detailMuted}>未被任何分镜或流程引用</p>
                    )}
                    {selectedAssetReferences.length > 0 && (
                      <ul className={styles.detailReferenceList}>
                        {selectedAssetReferences.map((ref) => (
                          <li key={`${ref.refType}-${ref.refId}`} className={styles.detailReferenceItem}>
                            <span className={styles.detailReferenceType}>
                              {ref.refType === 'storyboard_line' ? '分镜' : '流程输出'}
                            </span>
                            <span className={styles.detailReferenceTitle}>{ref.title}</span>
                            <span className={styles.detailReferenceProject}>{ref.projectName}</span>
                          </li>
                        ))}
                      </ul>
                    )}
      ...
      {deleteConfirmAsset && (
        <div className={styles.deleteConfirmOverlay} role="dialog" aria-modal="true" onClick={handleCancelDelete}>
          <div className={styles.deleteConfirmDialog} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.deleteConfirmBody}>
              <p>
                确定要删除 <strong>「{deleteConfirmAsset.name}」</strong> 吗？
              </p>
              {isCheckingReferences && (
                <p className={styles.deleteConfirmLoading}>
                  <Loader2 size={16} className={styles.spinnerIcon} /> 正在检查引用关系...
                </p>
              )}
              {!isCheckingReferences && deleteReferences.length > 0 && (
                <div className={styles.deleteConfirmReferences}>
                  <p className={styles.deleteConfirmWarning}>
                    <AlertTriangle size={16} />
                    该素材正在被以下 {deleteReferences.length} 处引用，删除可能导致相关内容异常：
                  </p>
                  <ul className={styles.deleteConfirmRefList}>
                    {deleteReferences.slice(0, 5).map((ref) => (
                      <li key={`${ref.refType}-${ref.refId}`}>
                        <span className={styles.deleteConfirmRefType}>
                          {ref.refType === 'storyboard_line' ? '分镜' : '流程'}
                        </span>
                        <span>{ref.title}</span>
                        <span className={styles.deleteConfirmRefProject}>{ref.projectName}</span>
                      </li>
                    ))}
                    {deleteReferences.length > 5 && (
                      <li className={styles.deleteConfirmMore}>
                        ...还有 {deleteReferences.length - 5} 处引用
                      </li>
                    )}
                  </ul>
                </div>
            ...
            <div className={styles.deleteConfirmActions}>
              ...
              {deleteReferences.length > 0 && !isCheckingReferences && (
                <button
                  type="button"
                ...
                >
                  {isDeleting ? (
                    <>
                      <Loader2 size={16} className={styles.spinnerIcon} /> 删除中...
                    </>
                  ) : (
                    <>强制删除</>
                  )}
