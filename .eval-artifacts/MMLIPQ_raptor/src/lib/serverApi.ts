...
export type ServerPrecheckResult = {
  projectId: string;
  projectName: string;
  canExport: boolean;
  issues: Array<{ severity: string; code: string; message: string; assetId?: string; assetName?: string }>;
  summary: {
    totalAssets: number;
    readyAssets: number;
...
export type ServerExportResult = {
  ...
  missingAssets: Array<Record<string, unknown>>;
 ...
 * 服务端导出预检
 */
export async function precheckServerExport(projectId: string): Promise<ServerPrecheckResult> {
  return requestApi<ServerPrecheckResult>('/api/exports/precheck', {
    method: 'POST',
    body: JSON.stringify({ projectId, exportType: 'full' }),
  });
}
 ...
 * 服务端创建可审计导出包
 */
export async function createServerExport(params: {
  projectId: string;
  exportType?: string;
  exportFormat?: string;
  snapshot?: Record<string, unknown>;
  coreMarkdown?: string;
  conversationsMarkdown?: Record<string, string>;
  assetPackaging?: Array<{ assetId: string; packagedPath: string }>;
  notes?: string;
}): Promise<ServerExportResult> {
  return requestApi<ServerExportResult>('/api/exports', {
    method: 'POST',
    body: JSON.stringify({
      projectId: params.projectId,
      exportType: params.exportType || 'full',
      exportFormat: params.exportFormat || 'tar.gz',
      snapshot: params.snapshot,
      coreMarkdown: params.coreMarkdown,
      conversationsMarkdown: params.conversationsMarkdown,
      assetPackaging: params.assetPackaging,
      notes: params.notes,
    }),
  });
}
 ...
 * 下载服务端导出包
 */
export async function downloadServerExport(auditId: string): Promise<Blob> {
  return requestApiBlob(`/api/exports/${auditId}/download`);
}
...
): Promise<{ records: ExportAuditRecord[]; limit: number; offset: number }> {
  const search = new URLSearchParams();
  if (params.limit) search.set('limit', String(params.limit));
  if (params.offset) search.set('offset', String(params.offset));
  const qs = search.toString();
  return requestApi(`/api/exports/project/${projectId}${qs ? `?${qs}` : ''}`);
}
...
export async function listMyExports(
  ...
  if (params.offset) search.set('offset', String(params.offset));
  const qs = search.toString();
  return requestApi(`/api/exports${qs ? `?${qs}` : ''}`);
}

/**
 * 获取导出详情
 */
export async function getExportDetail(auditId: string): Promise<Record<string, unknown>> {
  return requestApi(`/api/exports/${auditId}`);
}
