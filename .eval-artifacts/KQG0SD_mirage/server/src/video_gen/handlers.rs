...
pub async fn create_generation(
    ...
    // 预算检查（在扣减积分前进行）
    let budget_outcome = crate::budget::repo::check_budget_for_operation(
        &state.db,
        &user_id.0,
        "video",
        Some("video"),
    ...
    if let crate::budget::repo::BudgetCheckOutcome::Blocked { reason, .. } = budget_outcome {
        return Err(AppError::BudgetExceeded(reason));
    }
