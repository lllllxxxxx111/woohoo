...
/// 估算文本聊天/任务的积分消耗（简单估算：基于输入字符数）
/// 这是一个粗粒度估算，用于预算预警/拦截高成本任务
pub(crate) fn estimate_text_cost(input_chars: i64, max_tokens: Option<i64>) -> f64 {
    // 基础消耗
    let base = 0.05;
    // 每 1000 字符约消耗 0.05 积分
    let char_cost = (input_chars as f64 / 1000.0) * 0.05;
    // max_tokens 因子（如果设置了大输出窗口，预估成本提高）
    let token_factor = match max_tokens {
        Some(t) if t > 4096 => 1.5,
        Some(t) if t > 8192 => 2.0,
        _ => 1.0,
    };
    (base + char_cost) * token_factor
}

/// 在执行 AI 操作前检查预算，返回 Ok(()) 表示允许继续，Err 表示被拦截
pub(crate) async fn check_budget_before_ai(
    pool: &SqlitePool,
    user_id: &str,
    operation: &str,
    resource_kind: &str,
    ...
    let outcome = crate::budget::repo::check_budget_for_operation(
        pool,
        user_id,
        operation,
        Some(resource_kind),
    ...
    match outcome {
        crate::budget::repo::BudgetCheckOutcome::Allowed { status } => {
            // 如果有预警消息，记录到日志（前端可通过 /api/budget/status 轮询获取）
            if let Some(warn) = &status.warning_message {
                tracing::info!(user_id = %user_id, "Budget warning: {}", warn);
            }
            Ok(())
        }
        crate::budget::repo::BudgetCheckOutcome::Blocked { reason, .. } => {
