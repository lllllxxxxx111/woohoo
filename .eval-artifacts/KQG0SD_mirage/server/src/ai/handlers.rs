...
pub async fn ai_chat(
    ...
    // 预算检查（在持久化用户消息和调用 AI 之前）
    let estimated_cost = estimate_text_cost(
        context.content.chars().count() as i64,
        context.max_tokens,
    );
    check_budget_before_ai(
        &state.db,
        &user_id.0,
        AiUsageOperation::Chat.as_str(),
        context.output_kind.as_str(),
        estimated_cost,
        Some(&context.model),
        Some(&context.endpoint.id),
    )
    .await?;
...
pub async fn ai_chat_stream(
    ...
    check_budget_before_ai(
        &state.db,
        &user_id.0,
        AiUsageOperation::Stream.as_str(),
        context.output_kind.as_str(),
        estimated_cost,
        Some(&context.model),
        Some(&context.endpoint.id),
