...
import { ApiError, getAssetReferences, getStoredServerProfile, getServerAssetBlob, searchAssetsAcrossProjects, updateAssetTags as apiUpdateAssetTags } from '../../../../lib/serverApi';
...
export const AssetLibrary: React.FC = () => {
  const { activeState, assets, projects, assetLibraryView, setAssetLibraryView, isAuthenticated } =
    useAppStore(
    useShallow((state) => ({
  ...
  const [crossProjectResults, setCrossProjectResults] = useState<AssetWithProject[] | null>(null);
  const [isSearchingCrossProject, setIsSearchingCrossProject] = useState(false);
  const [assetReferences, setAssetReferences] = useState<AssetReference[] | null>(null);
  ...
  const sortMode = assetLibraryView.sortMode;
  const tagFilter = assetLibraryView.tagFilter;
  ...
  // Load references when an asset is selected for detail view
  useEffect(() => {
    if (!selectedAsset) {
      setAssetReferences(null);
      return;
    }
    let cancelled = false;
    setIsLoadingReferences(true);
    void getAssetReferences(selectedAsset)
      .then((res) => {
        if (!cancelled) setAssetReferences(res.references);
      })
      .catch(() => {
        if (!cancelled) setAssetReferences([]);
      })
      .finally(() => {
  ...
  // Initialize tags buffer when entering tag edit mode
  useEffect(() => {
    if (editingTags && selectedAssetData) {
      setTagsBuffer(getAssetTags(selectedAssetData.metadata));
      setTagInputValue('');
    }
  }, [editingTags, selectedAssetData]);
  ...
  const scopedAssets = useMemo(() => {
    ...
    // When cross-project results are available from backend search, merge them
    // with local assets to ensure we display projectName on cross-project results
    if (crossProjectResults && libraryScope === 'all') {
      return crossProjectResults as Asset[];
    }
    return assets;
  }, [activeState.projectId, assets, crossProjectResults, libraryScope]);

  const activeProjectName = activeState.projectId
    ? projectNameById.get(activeState.projectId) || '当前项目'
    : '未选择项目';

  // Aggregate all tags from the scoped assets for tag cloud/filter
  const availableTags = useMemo(
    () => aggregateAssetTags(scopedAssets),
    [scopedAssets],
  );

  // Cross-project backend search effect
  useEffect(() => {
    if (libraryScope !== 'all') {
      setCrossProjectResults(null);
      return;
    }

    // If no filters are active and no projectId filter, use local assets (already loaded via bootstrap)
    ...
    let cancelled = false;
    setIsSearchingCrossProject(true);
    ...
    const params = {
      ...
      ratingMin: ratingFilter > 0 ? ratingFilter : undefined,
      tag: tagFilter ?? undefined,
      sort: sortMode === 'recent' ? undefined : sortMode,
      limit: 200,
    };

    searchAssetsAcrossProjects(params)
      .then((results) => {
        if (!cancelled) setCrossProjectResults(results);
      })
      .catch((error) => {
        if (!cancelled) {
          ...
          if (error instanceof Error) {
            showToast({
              type: 'warning',
              title: '跨项目搜索降级',
              message: '后端搜索不可用，使用本地筛选。',
            });
          }
        }
      })
      .finally(() => {
        if (!cancelled) setIsSearchingCrossProject(false);
      });

    return () => {
      cancelled = true;
    };
  }, [libraryScope, searchQuery, filterType, favoriteOnly, ratingFilter, tagFilter, projectFilter, sortMode, showToast]);

  const filteredAssets = useMemo(() => {
    // When cross-project backend results are active, they're already filtered server-side
    if (crossProjectResults && libraryScope === 'all') {
      const sorted = sortAssetsClientSide(crossProjectResults as Asset[], sortMode);
      return sorted;
    }

    // Client-side filtering for current-project scope or when backend search is not active
    let results = filterAssetsClientSide(scopedAssets, {
      filterType,
      favoriteOnly,
      ratingMin: ratingFilter > 0 ? ratingFilter : undefined,
      searchQuery,
      tagFilter,
      projectId: libraryScope === 'current' ? activeState.projectId : projectFilter,
    });

    results = sortAssetsClientSide(results, sortMode);
    return results;
  }, [
  ...
  const handleAssetDelete = async (asset: Asset, event: React.MouseEvent) => {
    ...
    // First, load references if not already loaded
    let refs: AssetReference[];
    if (assetReferences && selectedAsset === asset.id) {
      refs = assetReferences;
    } else {
      try {
        const res = await getAssetReferences(asset.id);
        refs = res.references;
      } catch {
        // If we can't load references, block the delete for safety
        showToast({
          type: 'error',
          title: '无法检查引用关系',
          message: '删除前需确认素材引用状态，请稍后重试。',
        });
        return;
      }
    }
    ...
    if (refs.length > 0) {
      // Show confirmation dialog with reference impact
      setDeleteConfirmState({ asset, references: refs });
      return;
    }

    // No references - proceed with normal delete
    await performDelete(asset, false);
  };

  const performDelete = async (asset: Asset, force: boolean) => {
    setIsDeleting(true);
    try {
      ...
      showToast({
        type: 'success',
        title: force ? '素材已强制删除' : '资产已删除',
        message: force
          ? `${asset.name} 已删除，相关引用已解除。`
          : `${asset.name} 已从项目资产库移除。`,
      });
      setSelectedAsset((prev) => (prev === asset.id ? null : prev));
      setDeleteConfirmState(null);
    } catch (error) {
      // If server returns 409 with reference data, show the confirmation dialog
      if (error instanceof ApiError && error.status === 409 && isAssetDeleteBlockedError(error.body)) {
        setDeleteConfirmState({ asset, references: error.body.references });
        showToast({
          type: 'warning',
          title: '素材被引用',
  ...
  const handleTagAdd = () => {
    ...
    if (!tag) return;
    if (!tagsBuffer.some((t) => t.toLowerCase() === tag.toLowerCase())) {
      setTagsBuffer((prev) => [...prev, tag]);
    }
    setTagInputValue('');
  };

  const handleTagRemove = (tag: string) => {
    setTagsBuffer((prev) => prev.filter((t) => t !== tag));
  };

  const handleTagSave = async () => {
    if (!selectedAssetData) return;
    try {
      await apiUpdateAssetTags(selectedAssetData.id, tagsBuffer);
      // Update local asset metadata
      updateAsset(selectedAssetData.id, {
        metadata: { ...(selectedAssetData.metadata ?? {}), tags: tagsBuffer },
      });
      setEditingTags(false);
      showToast({
        type: 'success',
        title: '标签已更新',
        message: `已更新「${selectedAssetData.name}」的标签。`,
      });
    } catch (error) {
      showToast({
        type: 'error',
        title: '标签更新失败',
        message: error instanceof Error ? error.message : '更新标签时发生错误',
      });
    }
  };
    ...
    <div className={styles.container}>
      <div className={styles.topToolbar}>
        ...
        <div className={styles.actions}>
          ...
          <div className={styles.filterDropdown}>
            <button
              className={`${styles.toolBtn} ${tagFilter ? styles.activeToolBtn : ''}`}
              onClick={() => {
            ...
            {showTagDropdown && (
              <div className={styles.filterMenu}>
                <button
                  className={`${styles.filterItem} ${!tagFilter ? styles.active : ''}`}
                  onClick={() => {
                    updateLibraryView({ tagFilter: null });
                ...
                {availableTags.slice(0, 20).map(({ tag, count }) => (
                  <button
                    key={tag}
                    className={`${styles.filterItem} ${tagFilter === tag ? styles.active : ''}`}
                    onClick={() => {
                      updateLibraryView({ tagFilter: tag });
                      setShowTagDropdown(false);
                    }}
                  >
                    <Tag size={14} />
                ...
                {availableTags.length === 0 && (
                  <div className={styles.filterEmpty}>暂无标签</div>
                )}
              </div>
            )}
          </div>

          {/* Project filter (cross-project scope only) */}
          {libraryScope === 'all' && (
            <div className={styles.filterDropdown}>
              <button
                className={`${styles.toolBtn} ${projectFilter ? styles.activeToolBtn : ''}`}
                onClick={() => {
              ...
              >
                <FolderOpen size={16} />
                {projectFilter ? projectNameById.get(projectFilter) ?? '项目' : '项目'}
              </button>
              {showProjectFilter && (
                <div className={styles.filterMenu}>
                  <button
                    className={`${styles.filterItem} ${!projectFilter ? styles.active : ''}`}
                    onClick={() => {
                  ...
                  {projects.map((project) => (
                    <button
                      key={project.id}
                      className={`${styles.filterItem} ${projectFilter === project.id ? styles.active : ''}`}
                      onClick={() => {
          ...
          <div className={styles.filterDropdown}>
            <button
              className={styles.toolBtn}
              onClick={() => {
                const modes: AssetLibrarySortMode[] = ['recent', 'name', 'rating'];
                const idx = modes.indexOf(sortMode);
                const next = modes[(idx + 1) % modes.length];
                setAssetLibraryView({ sortMode: next });
              }}
              aria-label="排序方式"
              title={`排序: ${ASSET_SORT_LABELS[sortMode]}`}
            >
              {ASSET_SORT_LABELS[sortMode]}
            </button>
          </div>

          {isSearchingCrossProject && (
            ...
            <aside className={`${styles.detailInfoPanel} ${isDetailInfoCollapsed ? styles.collapsedInfoPanel : ''}`}>
              ...
              {!isDetailInfoCollapsed && (
                <div className={styles.detailInfoContent}>
                  ...
                  {/* Tags section */}
                  <section className={styles.detailTagsBlock}>
                    <div className={styles.detailReviewHeader}>
                      <span><Tag size={14} /> 标签</span>
                      {!editingTags && (
                        <button
                          type="button"
                          className={styles.detailInlineBtn}
                    ...
                    {editingTags ? (
                      <div className={styles.tagEditor}>
                        <div className={styles.tagEditorInput}>
                          <input
                            type="text"
                            value={tagInputValue}
                            ...
                            onKeyDown={(e) => {
                              ...
                              if (e.key === 'Backspace' && !tagInputValue && tagsBuffer.length > 0) {
                                handleTagRemove(tagsBuffer[tagsBuffer.length - 1]);
                              }
                            }}
                        ...
                        <div className={styles.tagEditorActions}>
                          ...
                          <button type="button" className={styles.tagSaveBtn} onClick={() => void handleTagSave()}>
                            <Check size={14} /> 保存
                          </button>
                        </div>
                      </div>
                      ...
                      <div className={styles.tagList}>
                        {getAssetTags(selectedAssetData.metadata).length > 0 ? (
                          getAssetTags(selectedAssetData.metadata).map((tag) => (
                            <span
                              key={tag}
                              className={styles.tagChip}
                              onClick={() => {
                                updateLibraryView({ tagFilter: tag, scope: 'all' });
                                setSelectedAsset(null);
                              }}
                              title="点击筛选此标签"
                            >
                          ...
                          <span className={styles.detailMuted}>暂无标签</span>
                        )}
                      </div>
                    )}
                  </section>
                  ...
                  {/* References section */}
                  <section className={styles.detailReferencesBlock}>
                    <div className={styles.detailReviewHeader}>
                      <span><Link2 size={14} /> 引用关系</span>
                      {isLoadingReferences && <Loader2 size={14} className={styles.previewSpinner} />}
                    </div>
                    {assetReferences && assetReferences.length > 0 ? (
                      <div className={styles.referenceList}>
                        {(() => {
                          const grouped = groupReferencesByType(assetReferences);
                          return (
                            <>
                              {grouped.storyboard.length > 0 && (
                                <div className={styles.referenceGroup}>
                                  <div className={styles.referenceGroupTitle}>
                                    {REFERENCE_TYPE_LABELS.storyboard} ({grouped.storyboard.length})
                                  </div>
                                  {grouped.storyboard.slice(0, 5).map((ref, i) => (
                                    <div key={`sb-${i}`} className={styles.referenceItem}>
                                      <span className={styles.referenceProject}>{ref.projectName}</span>
                                      <span className={styles.referenceTitle}>{ref.title}</span>
                                    </div>
                                  ))}
                                </div>
                              )}
                              {grouped.pipelineStep.length > 0 && (
                                <div className={styles.referenceGroup}>
                                  <div className={styles.referenceGroupTitle}>
                                    {REFERENCE_TYPE_LABELS.pipelineStep} ({grouped.pipelineStep.length})
                                  </div>
                                  {grouped.pipelineStep.slice(0, 5).map((ref, i) => (
                                    <div key={`ps-${i}`} className={styles.referenceItem}>
                                      <span className={styles.referenceProject}>{ref.projectName}</span>
                                      <span className={styles.referenceTitle}>{ref.title}</span>
                                    </div>
                                  ))}
                              ...
                              {grouped.pipelineStepInput.length > 0 && (
                                <div className={styles.referenceGroup}>
                                  <div className={styles.referenceGroupTitle}>
                                    {REFERENCE_TYPE_LABELS.pipelineStepInput} ({grouped.pipelineStepInput.length})
                                  </div>
                                  {grouped.pipelineStepInput.slice(0, 5).map((ref, i) => (
                                    <div key={`psi-${i}`} className={styles.referenceItem}>
                                      <span className={styles.referenceProject}>{ref.projectName}</span>
                                      <span className={styles.referenceTitle}>{ref.title}</span>
                                    </div>
                                  ))}
                              ...
                              <div className={styles.referenceSummary}>
                                {buildReferenceSummary(assetReferences)}
                              </div>
                            </>
                          );
                        })()}
                      ...
                      <p className={styles.detailMuted}>无引用，可安全删除</p>
                    ) : (
                      <p className={styles.detailMuted}>加载中...</p>
                    )}
                  </section>
                </div>
              )}
        ...
        >
          ...
          >
            <div className={styles.deleteConfirmHeader}>
              <AlertTriangle size={24} className={styles.deleteConfirmIcon} />
              <h3 id="delete-confirm-title">确认删除素材</h3>
            </div>
            <div className={styles.deleteConfirmBody}>
              <p>
                素材「<strong>{deleteConfirmState.asset.name}</strong>」当前被{' '}
                <strong>{deleteConfirmState.references.length}</strong> 处引用：
              </p>
              <div className={styles.deleteConfirmRefList}>
                {(() => {
                  const grouped = groupReferencesByType(deleteConfirmState.references);
                  return (
                    <>
                      {grouped.storyboard.length > 0 && (
                        <div className={styles.deleteConfirmRefGroup}>
                          <div className={styles.deleteConfirmRefGroupTitle}>
                            {REFERENCE_TYPE_LABELS.storyboard} ({grouped.storyboard.length})
                          </div>
                          {grouped.storyboard.slice(0, 3).map((ref, i) => (
                            <div key={`dcsb-${i}`} className={styles.deleteConfirmRefItem}>
                              <span className={styles.deleteConfirmRefProject}>{ref.projectName}</span>
                              <span>{ref.title}</span>
                            </div>
                          ))}
                          {grouped.storyboard.length > 3 && (
                            <div className={styles.deleteConfirmRefMore}>
                              ...还有 {grouped.storyboard.length - 3} 处
                            </div>
                          )}
                        </div>
                      )}
                      {grouped.pipelineStep.length > 0 && (
                        <div className={styles.deleteConfirmRefGroup}>
                          ...
                          {grouped.pipelineStep.slice(0, 3).map((ref, i) => (
                            <div key={`dcps-${i}`} className={styles.deleteConfirmRefItem}>
                              <span className={styles.deleteConfirmRefProject}>{ref.projectName}</span>
                              <span>{ref.title}</span>
                            </div>
                          ))}
                          {grouped.pipelineStep.length > 3 && (
                            <div className={styles.deleteConfirmRefMore}>
                              ...还有 {grouped.pipelineStep.length - 3} 处
                            </div>
                          )}
                        </div>
                      )}
                      {grouped.pipelineStepInput.length > 0 && (
                        <div className={styles.deleteConfirmRefGroup}>
                          ...
                          {grouped.pipelineStepInput.slice(0, 3).map((ref, i) => (
                            <div key={`dcpsi-${i}`} className={styles.deleteConfirmRefItem}>
                              <span className={styles.deleteConfirmRefProject}>{ref.projectName}</span>
                              <span>{ref.title}</span>
                            </div>
                          ))}
              ...
              <p className={styles.deleteConfirmWarning}>
                删除后，分镜中的关联将被移除。流水线历史记录中对该素材的引用将变为悬空引用，但历史记录本身保留。
              </p>
            </div>
            <div className={styles.deleteConfirmActions}>
              ...
              <button
                type="button"
                className={styles.deleteConfirmForce}
                onClick={() => void performDelete(deleteConfirmState.asset, true)}
                disabled={isDeleting}
              >
                {isDeleting ? (
                  <>
                    <Loader2 size={16} className={styles.previewSpinner} />
                    删除中...
                  </>
                ) : (
                  <>
