...
export function useChatWorkspaceActions({
  ...
  const deleteAsset = useCallback(
    async (assetId: string, options?: { force?: boolean; skipServerCall?: boolean }) => {
      ...
      if (!options?.skipServerCall) {
        try {
          await deleteServerAsset(assetId, options?.force ?? false);
        } catch (error) {
          logger.error('Failed to delete asset on server', error);
          throw error; // Re-throw so caller can handle reference errors
        }
      }
