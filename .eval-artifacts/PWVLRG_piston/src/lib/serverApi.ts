...
/**
 * Structured API error that preserves status code and parsed response body.
 * Extends Error so existing `catch (e) { if (e instanceof Error) ... }` patterns work.
 */
export class ApiError extends Error {
  ...
  constructor(message: string, status: number, body: unknown, requestId?: string | null) {
    super(appendRequestId(message, requestId));
    this.name = 'ApiError';
    this.status = status;
    this.body = body;
  }
...
async function parseResponse<T>(response: Response): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(errorMessage, response.status, parsed, requestId);
  }
...
export async function deleteServerAsset(assetId: string, force = false) {
  ...
  await requestApi<void>(`/api/assets/${assetId}${qs}`, {
    method: 'DELETE',
  });
...
/**
 * Fetch reference information for an asset (storyboard, pipeline, etc.).
 */
export async function getAssetReferences(assetId: string) {
  return requestApi<import('../types').AssetReferencesResponse>(`/api/assets/${assetId}/references`);
}

/**
 * Cross-project asset search. Uses the backend search endpoint with full-text
 * search across name, project name, and metadata fields (prompt, summary, description, tags).
 */
export async function searchAssetsAcrossProjects(params: import('../types').AssetSearchParams) {
  ...
  if (params.ratingMin && params.ratingMin > 0) searchParams.set('ratingMin', String(params.ratingMin));
  ...
  const path = `/api/assets/search${qs ? `?${qs}` : ''}`;

  type ServerAssetWithProject = {
    id: string;
    projectId: string;
    projectName: string;
    name: string;
    type: import('../types').Asset['type'];
    url: string;
    metadata?: Record<string, unknown> | string | null;
    createdAt: string | number;
    updatedAt?: string | number;
  ...
  const results = await requestApi<ServerAssetWithProject[]>(path);
  return results.map((a) => ({
    ...mapAsset({ ...a, ownerUserId: undefined }),
    projectName: a.projectName,
  }));
}

/**
 * Update tags for an asset. Tags are stored in metadata.tags as a JSON array.
 */
