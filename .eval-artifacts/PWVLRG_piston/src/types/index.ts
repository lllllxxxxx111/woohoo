...
export interface AssetDeleteBlockedError {
  error: string;
...
export function isAssetDeleteBlockedError(err: unknown): err is AssetDeleteBlockedError {
  return (
    typeof err === 'object' &&
    err !== null &&
    'errorCode' in err &&
    (err as { errorCode: string }).errorCode === 'ASSET_HAS_REFERENCES'
  );
}

/**
 * Extract tags from asset metadata, normalizing to a clean string array.
 */
export function getAssetTags(metadata: Asset['metadata']): string[] {
