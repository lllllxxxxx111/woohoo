...
async fn main() {
    ...
    // 确保导出目录存在
    if let Err(error) = std::fs::create_dir_all(&config.exports_dir) {
        tracing::warn!("Failed to create exports dir {}: {}", config.exports_dir, error);
    } else {
        tracing::info!(exports_dir = %config.exports_dir, "导出目录已就绪");
    }
    ...
    let protected_routes = Router::new()
        ...
        // 导出审计包
        .route(
            "/api/projects/{project_id}/export/preflight",
            get(export_audit::handlers::preflight_export),
        )
        .route(
            "/api/projects/{project_id}/export",
            post(export_audit::handlers::create_export),
        )
        .route(
            "/api/export-audits",
            get(export_audit::handlers::list_export_audits),
        )
        .route(
            "/api/export-audits/{id}",
            get(export_audit::handlers::get_export_audit),
        )
        .route(
            "/api/export-audits/{id}/download",
            get(export_audit::handlers::download_export),
        )
        // AI
        .route(
            "/api/ai/endpoints",
