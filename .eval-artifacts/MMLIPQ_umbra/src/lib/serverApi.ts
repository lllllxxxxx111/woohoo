...
export interface ExportPreflightMissingAsset {
  assetId: string;
  name: string;
  type: string;
  reason: string;
...
export interface ExportPreflightResult {
  ...
  assetMissing: number;
  assetEmpty: number;
  assetExternalUrl: number;
  estimatedSizeBytes: number;
  missingAssets: ExportPreflightMissingAsset[];
  scriptPresent: boolean;
  scriptEmpty: boolean;
  scriptSizeBytes: number;
  storyboardPresent: boolean;
  storyboardEmptyScenes: number;
  storyboardDuplicateScenes: number[];
  conversationCount: number;
  duplicateFilenames: DuplicateFilenameGroup[];
  findings: PreflightFinding[];
  blockingCount: number;
  warningCount: number;
  infoCount: number;
  warnings: string[]; // backwards compat
  ready: boolean;
}

export interface ExportAuditChecksum {
  assetId: string;
  archivePath: string;
  sha256: string;
  sizeBytes: number;
...
export interface ExportAuditDetail {
  ...
  projectId: string;
  userId: string;
  exportType: 'full' | 'core' | 'final_cut';
  ...
  checksums: ExportAuditChecksum[];
  verificationReport: {
    verifiedAt: string;
    archiveSha256: string;
    archiveSizeBytes: number;
    fileCount: number;
    assetChecksumsVerified: number;
    assetChecksumsFailed: number;
    missingAssets: ExportPreflightMissingAsset[];
...
export interface ExportAuditListItem {
  id: string;
  projectId: string;
  projectName?: string;
  exportType: string;
...
export interface ExportAuditListResponse {
  items: ExportAuditListItem[];
  total: number;
  limit: number;
  offset: number;
}

export async function preflightExport(projectId: string): Promise<ExportPreflightResult> {
  return requestApi<ExportPreflightResult>(
    `/api/projects/${projectId}/export/preflight`,
  );
}

export async function createAuditedExport(
  projectId: string,
  exportType: 'full' | 'core' = 'full',
  note?: string,
): Promise<ExportAuditDetail> {
  return requestApi<ExportAuditDetail>(`/api/projects/${projectId}/export`, {
    method: 'POST',
    body: JSON.stringify({ exportType, note }),
  });
}
...
export async function listExportAudits(
  ...
  limit = 20,
  offset = 0,
): Promise<ExportAuditListResponse> {
  const params = new URLSearchParams();
  if (projectId) params.set('projectId', projectId);
  params.set('limit', String(limit));
  params.set('offset', String(offset));
  return requestApi<ExportAuditListResponse>(`/api/export-audits?${params.toString()}`);
}

export async function getExportAudit(auditId: string): Promise<ExportAuditDetail> {
  return requestApi<ExportAuditDetail>(`/api/export-audits/${auditId}`);
}

export async function downloadExportArchive(auditId: string): Promise<Blob> {
  return requestApiBlob(`/api/export-audits/${auditId}/download`);
}
