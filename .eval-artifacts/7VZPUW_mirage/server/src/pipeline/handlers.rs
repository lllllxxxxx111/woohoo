...
/**
 * 获取人工复核队列
 * 聚合当前用户下 failed、blocked 状态的步骤，以及有 prompt optimization 或需要人工复核的步骤
...
pub async fn get_review_queue(
    ...
    // Build the review queue: steps that are failed/blocked, or runs with MANUAL_REVIEW_REQUIRED,
    // or steps with prompt optimizations
    let mut conditions = vec![
        "r.user_id = ?".to_string(),
        "s.status IN ('failed', 'blocked')".to_string(),
    ];
    let mut values: Vec<String> = vec![user_id.0.clone()];
    ...
    let query = format!(
        r#"SELECT
            ...
            r.pipeline_type AS pipeline_type,
            ...
            s.max_retries AS max_retries,
            s.error_message AS error_message,
            LAST_EVENT.event_type AS last_event_type,
            LAST_EVENT.payload_json AS last_event_payload,
            LAST_EVENT.created_at AS last_event_at,
            COALESCE(OPT.opt_count, 0) AS optimization_count,
            COALESCE(REV.review_count, 0) AS review_count,
            LAST_REV.decision AS last_review_decision,
            LAST_REV.created_at AS last_review_at,
        ...
        LEFT JOIN (
            SELECT run_id, step_id, event_type, payload_json, created_at,
                   ROW_NUMBER() OVER (PARTITION BY run_id, step_id ORDER BY created_at DESC, rowid DESC) as rn
            FROM pipeline_run_events
            WHERE step_id IS NOT NULL
        ) LAST_EVENT ON LAST_EVENT.run_id = r.id AND LAST_EVENT.step_id = s.id AND LAST_EVENT.rn = 1
        LEFT JOIN (
            SELECT run_id, step_id, COUNT(*) as opt_count
            FROM pipeline_prompt_optimizations
            GROUP BY run_id, step_id
        ) OPT ON OPT.run_id = r.id AND OPT.step_id = s.id
        LEFT JOIN (
            SELECT run_id, step_id, COUNT(*) as review_count,
                   MAX(created_at) as last_review_at
            FROM pipeline_manual_reviews
            GROUP BY run_id, step_id
        ) REV ON REV.run_id = r.id AND REV.step_id = s.id
        LEFT JOIN (
            SELECT run_id, step_id, decision, created_at,
                   ROW_NUMBER() OVER (PARTITION BY run_id, step_id ORDER BY created_at DESC) as rn
            FROM pipeline_manual_reviews
        ) LAST_REV ON LAST_REV.run_id = r.id AND LAST_REV.step_id = s.id AND LAST_REV.rn = 1
        WHERE {}
    ...
    let mut q = sqlx::query_as::<_, ReviewQueueItem>(&query);
    for v in &values {
        q = q.bind(v);
    }
    q = q.bind(limit).bind(offset);
...
/**
 * 获取步骤复核详情（含最近事件、优化建议、历史复核记录）
 * GET /api/pipelines/runs/{runId}/steps/{stepId}/review-detail
 */
pub async fn get_step_review_detail(
    State(state): State<AppState>,
    Extension(user_id): Extension<UserId>,
    Path((run_id, step_id)): Path<(String, String)>,
) -> AppResult<Json<StepReviewDetail>> {
    ...
    let events = sqlx::query_as::<_, PipelineRunEvent>(
        "SELECT * FROM pipeline_run_events WHERE run_id = ? AND (step_id = ? OR step_id IS NULL) ORDER BY created_at DESC, rowid DESC LIMIT 30",
    )
    .bind(&run_id)
    .bind(&step_id)
    .fetch_all(&state.db)
    ...
    let outputs = sqlx::query_as::<_, PipelineStepOutput>(
        "SELECT * FROM pipeline_step_outputs WHERE run_id = ? AND step_id = ? ORDER BY created_at DESC LIMIT 20",
    )
    .bind(&run_id)
    .bind(&step_id)
    .fetch_all(&state.db)
    ...
    let optimizations = sqlx::query_as::<_, PipelinePromptOptimization>(
        "SELECT * FROM pipeline_prompt_optimizations WHERE run_id = ? AND step_id = ? ORDER BY created_at DESC LIMIT 20",
    )
    .bind(&run_id)
    .bind(&step_id)
    .fetch_all(&state.db)
...
pub async fn submit_review_decision(
    ...
    // Verify ownership and load run + step
    let run = get_run_by_id(&state.db, &user_id.0, &run_id).await?;
    let step = sqlx::query_as::<_, PipelineRunStep>(
        "SELECT * FROM pipeline_run_steps WHERE id = ? AND run_id = ?",
    )
    ...
    // Reject actions on terminal runs
    ...
    // Execute the decision's side effects
    let mut updated_run: Option<PipelineRun> = None;
    let mut updated_step: Option<PipelineRunStep> = None;

    match decision.as_str() {
        "retry" => {
            // Reuse retry-step semantics: set step to retrying, resume run if failed
            if !["running", "paused", "failed"].contains(&run.status.as_str()) {
                return Err(AppError::Validation(
                    format!("当前流程状态不允许重试步骤: {}", run.status).into(),
                ));
            }
            if !["failed", "blocked"].contains(&step.status.as_str()) {
                return Err(AppError::Validation(
        ...
        "cancel" => {
            // Cancel the entire run (safe from queued/running/paused/failed)
            let (run_after_cancel, changed) = if ["queued", "running", "paused"].contains(&run.status.as_str()) {
                cancel_pipeline_run_state(&state.db, &user_id.0, &run_id).await?
            } else if run.status == "failed" {
                // Already failed, move to cancelled
                let cancelled = sqlx::query_as::<_, PipelineRun>(
                    "UPDATE pipeline_runs
            ...
            updated_run = Some(run_after_cancel.clone());

            if changed {
                // Mark any non-terminal steps as skipped to leave no dangling running steps
                sqlx::query(
                    "UPDATE pipeline_run_steps
                     SET status = 'skipped', updated_at = ?
                     WHERE run_id = ? AND status IN ('queued', 'running', 'retrying', 'blocked')",
                )
                .bind(&now)
                .bind(&run_id)
                .execute(&state.db)
            ...
            let _ = run_after_cancel;
        }
        "acknowledge" => {
            // Just record the review, no state change
            log_pipeline_event(
                &state.db,
                &run_id,
                Some(&step_id),
...
pub struct ReviewDecisionResponse {
    ...
    pub step: Option<PipelineRunStep>,
}
...
mod tests {
    use super::{
        ...
        PipelineRunFilter, ReviewQueueFilter, ReviewDecisionReq,
    };
    ...
    #[test]
    fn review_queue_filter_accepts_camel_case_params() {
        let uri: Uri = "/api/pipelines/review-queue?projectId=proj-x&status=failed&pipelineType=outline&limit=20&offset=0"
            .parse()
            .expect("valid test uri");
        let Query(filter) =
            Query::<ReviewQueueFilter>::try_from_uri(&uri).expect("query should parse");

        assert_eq!(filter.project_id.as_deref(), Some("proj-x"));
    ...
    #[test]
    fn review_queue_filter_accepts_snake_case_project_id() {
        let uri: Uri = "/api/pipelines/review-queue?project_id=proj-y&status=blocked"
            .parse()
            .expect("valid test uri");
        let Query(filter) =
            Query::<ReviewQueueFilter>::try_from_uri(&uri).expect("query should parse");

        assert_eq!(filter.project_id.as_deref(), Some("proj-y"));
        assert_eq!(filter.status.as_deref(), Some("blocked"));
        assert_eq!(filter.pipeline_type, None);
        assert_eq!(filter.limit, None);
    }

    #[test]
    fn review_queue_filter_handles_empty_params() {
        let uri: Uri = "/api/pipelines/review-queue"
            .parse()
            .expect("valid test uri");
        let Query(filter) =
            Query::<ReviewQueueFilter>::try_from_uri(&uri).expect("query should parse");
    ...
    #[test]
    fn review_decision_req_parses_valid_payloads() {
        let req: ReviewDecisionReq =
            serde_json::from_str(r#"{"decision":"retry","note":"fix prompt"}"#).unwrap();
        assert_eq!(req.decision, "retry");
        assert_eq!(req.note.as_deref(), Some("fix prompt"));

        let req: ReviewDecisionReq =
            serde_json::from_str(r#"{"decision":"cancel"}"#).unwrap();
        assert_eq!(req.decision, "cancel");
        assert_eq!(req.note, None);

        let req: ReviewDecisionReq =
            serde_json::from_str(r#"{"decision":"acknowledge","note":"  "}"#).unwrap();
        assert_eq!(req.decision, "acknowledge");
        // Empty/whitespace note should be treated as no note
        let trimmed = req.note.as_deref().map(str::trim).filter(|s| !s.is_empty());
        assert_eq!(trimmed, None);
    }

    #[tokio::test]
    async fn review_decision_rejects_skip() {
        // The backend intentionally does not support skip (see design rationale:
        // skipped in this state machine means dependency-failure, not manual override).
        // Verify that serde accepts it as a string but our handler validation rejects it.
        let req: ReviewDecisionReq =
            serde_json::from_str(r#"{"decision":"skip"}"#).unwrap();
        assert_eq!(req.decision, "skip");
        // The handler validates against ["retry", "cancel", "acknowledge"],
        // so "skip" would be rejected with a Validation error.
        let valid = ["retry", "cancel", "acknowledge"]
            .contains(&req.decision.to_lowercase().as_str());
        assert!(!valid);
    }
}
