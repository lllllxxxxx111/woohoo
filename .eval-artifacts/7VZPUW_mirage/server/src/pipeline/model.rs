...
#[derive(Debug, Clone, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ReviewDecisionReq {
    pub decision: String,
    pub note: Option<String>,
}

/**
 * 复核队列项：聚合了 run、step、最近事件、错误信息、优化建议数量
 */
...
/**
 * 步骤复核详情：包含 step 信息、最近事件、优化建议、历史复核记录
 */
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct StepReviewDetail {
    pub run: PipelineRun,
    pub step: PipelineRunStep,
    pub recent_events: Vec<PipelineRunEvent>,
    pub outputs: Vec<PipelineStepOutput>,
    pub optimizations: Vec<PipelinePromptOptimization>,
