...
import { normalizeAiSettingsPayload, requestAiCompletion } from '../../lib/ai';
...
export function useAiMessageRuntime({
  ...
  const getMessagesBeforeRewindAnchor = useCallback(
    (messages: Message[], anchorMessageId?: string) => {
      if (!anchorMessageId) {
        return messages;
      }

      const anchorIndex = messages.findIndex((message) => message.id === anchorMessageId);
      return anchorIndex >= 0 ? messages.slice(0, anchorIndex) : messages;
    },
    [],
  );
  ...
  const requestDirectAiReply = useCallback(
    async (
      settings: AiSettings,
      ...
      const referencedAssets = availableAssets.filter((asset) =>
        resourceRefs.some((resourceRef) => resourceRef.id === asset.id),
      );
      const referencedAssetPrompt =
        referencedAssets.length > 0
          ? [
              ...
              ...referencedAssets
                ...
                .map(
                  (asset) =>
                    `- ${asset.projectName || '未命名项目'} : ${formatAssetTypeLabel(asset.type)} : ${asset.name} : ${asset.versionLabel || '当前版'} | id ${asset.id} | 创建于 ${new Date(asset.createdAt).toLocaleString('zh-CN')}`,
                ),
            ].join('\n')
          : '';
      const systemPrompt = buildSystemPrompt(
        ...
        [
          buildProjectContextPrompt(
            project,
            allAgentContacts.length ? allAgentContacts : defaultAgents,
          ),
          referencedAssetPrompt,
        ]
          .filter(Boolean)
          .join('\n\n'),
      );
    ...
    [allAgentContacts, defaultAgents],
  );
  ...
  const sendAiMessage = useCallback(
    async (content: string, options?: SendMessageOptions): Promise<SendMessageResult> => {
      ...
      const requireServerTask = options?.requireServerTask ?? false;
      ...
      if (!usingProjectScope) {
        if (requireServerTask) {
          throw new Error('当前未进入项目，无法创建服务端任务，请先创建或选择项目。');
        }
        ...
        const historyMessages = buildHistoryMessagesFromList(globalChatMessages);
        ...
        try {
          const result = await requestDirectAiReply(
            normalizedSettings,
            historyMessages,
            normalizedContent,
            agent,
          ...
          updateGlobalMessageLocally(placeholderMessageId, (message) => ({
            ...message,
          ...
          updateGlobalMessageLocally(placeholderMessageId, (currentMessage) => ({
            ...currentMessage,
            role: 'system',
      ...
      const { targetProjectId, targetChatId, chatSnapshot } = await ensureChatTarget();
      const targetProject = projects.find((project) => project.id === targetProjectId);
      const projectAssets = assets.filter((asset) => asset.projectId === targetProjectId);
      const { sanitizedContent: normalizedContent, resourceRefs } = normalizeResourceMentions(
        sanitizedContent,
        assets,
        options?.resourceRefs,
      ...
      const attachments = options?.attachments ?? [];
      const rewindSourceMessages = getMessagesBeforeRewindAnchor(
        chatSnapshot?.messages ?? activeChat?.messages ?? [],
        options?.rewindFromMessageId,
      );
      const historyMessages = options?.rewindFromMessageId
        ? buildHistoryMessagesFromList(rewindSourceMessages)
        : buildHistoryMessages(chatSnapshot);
      const userMessageId = createId('msg');
      const triggerSource = options?.triggerSource;
      const initialUserMessageMeta: Message['meta'] =
        options?.confirmedWorkflowGuardMessageId ||
      ...
      try {
        ...
        if (requireServerTask) {
          throw new Error('服务端未就绪，当前操作要求必须通过服务端任务执行');
        }

        logger.warn('[sendAiMessage] Using direct AI mode (server not ready)');
        const result = await requestDirectAiReply(
          normalizedSettings,
          historyMessages,
          normalizedContent,
        ...
        updateMessageLocally(targetProjectId, targetChatId, placeholderMessageId, (message) => ({
          ...message,
          content: result.content,
          timestamp: Date.now(),
          agentId: agent?.id,
        ...
        if (options?.confirmedWorkflowGuardMessageId) {
          updateMessageLocally(targetProjectId, targetChatId, userMessageId, (message) => ({
            ...message,
            meta: {
              ...(message.meta ?? {}),
              confirmedWorkflowGuardMessageId: options.confirmedWorkflowGuardMessageId,
